// ArtsperrView.cpp: Implementierungsdatei
//
...
IMPLEMENT_DYNCREATE(CArtsperrView, CFormView)

CArtsperrView::CArtsperrView()
	: ChPflegeView(CArtsperrView::IDD)
{
	//{{AFX_DATA_INIT(CArtsperrView)
...
void CArtsperrView::DoDataExchange(CDataExchange* pDX)
{
	CFormView::DoDataExchange(pDX);
	//{{AFX_DATA_MAP(CArtsperrView)
	DDX_Control(pDX, IDC_ARTSPERR_DATBIS, m_DATUMBIS);
	DDX_Control(pDX, IDC_ARTSPERR_DATVON, m_DATUMVON);
	//}}AFX_DATA_MAP
}


BEGIN_MESSAGE_MAP(CArtsperrView, CFormView)
	//{{AFX_MSG_MAP(CArtsperrView)
	ON_WM_SIZE()
...
/////////////////////////////////////////////////////////////////////////////
// Diagnose CArtsperrView

#ifdef _DEBUG
void CArtsperrView::AssertValid() const
{
	CFormView::AssertValid();
}

void CArtsperrView::Dump(CDumpContext& dc) const
{
	CFormView::Dump(dc);
}
#endif //_DEBUG
...
void CArtsperrView::InitPflege()
{
	m_wndGrid.SubclassDlgItem(IDC_ARTSPERR_GRID, this );
...
void CArtsperrView::InitVariables()
{
	m_nDatumVon = 0;
	m_nDatumBis = 0;
...
bool CArtsperrView::ValidateSuchb()
{
...
void CArtsperrView::FirstInitGrid( )
{
...
void CArtsperrView::FillGrid( void *pp, int index )
{
	struct BUFF_ARTSPERR *p;
	ROWCOL Row;
	...
	p = (struct BUFF_ARTSPERR *) pp;

	if ( p->DATUM_AENDER > p->DATUM_IFAAENDER )
		datum = p->DATUM_AENDER;
	else
		datum = p->DATUM_IFAAENDER;
	...
	ConvFromPhoDate( dat_str, datum, "tt.mm.jjjj" );
	m_wndGrid.SetValueRange(CGXRange ( Row, 4 ), dat_str );
	m_wndGrid.SetValueRange(CGXRange ( Row, 5 ), p->NAHRUNGSERGAENZUNG );
...
void CArtsperrView::FillBox( int anz )
{
	void *p;
	int i;
...
CString CArtsperrView::GetSelect( )
{
	CString sql;

	sql.Format( "select lpad(trim(ARTICLE_CODE), %d, 0),einheit,artikel_name,a.datum_aender,a.datum_ifaaender,"\
		        "nahrungsergaenzung,a.hersteller_nr,name from " \
				...
				"(a.datum_aender>=%ld and a.datum_aender<=%ld or" \
				" a.datum_ifaaender>=%ld and a.datum_ifaaender<=%ld) and " \
				"h.hersteller_nr=a.hersteller_nr and nahrungsergaenzung='G' " \
...
void CArtsperrView::EnableOrder( BOOL mode )
{
...
int  CArtsperrView::BearbeiteSatz()
{
...
/////////////////////////////////////////////////////////////////////////////
// Behandlungsroutinen für Nachrichten CArtsperrView 

void CArtsperrView::OnInitialUpdate() 
{
...
void CArtsperrView::OnActivateView(BOOL bActivate, CView* pActivateView, CView* pDeactiveView) 
{
	// TODO: Add your specialized code here and/or call the base class
...
void CArtsperrView::OnSize(UINT nType, int cx, int cy) 
{
...
void CArtsperrView::ResizeControls( int cx, int cy ) 
{
...
void CArtsperrView::FillZeile( void *pp , CString& cStr )
{
	long datum;
	char dat_str[12];
	struct BUFF_ARTSPERR *p;
...
CString CArtsperrView::GetListHeader()
{
	return _T( "PZN        Einheit Artikelname                    " \
			   "Änder-Datum NEM HerstNr Herstellername"; );
}

CString CArtsperrView::GetListStrich()
{
	return _T( "-------------------------------------------------" \
		       "----------------------------------------------" );
}

bool CArtsperrView::IsPrintZeile()
{
	void* p;
...
void* CArtsperrView::GetPrintZeile()
{
	return m_hSelSql.GetBuffer( m_nSelectZeile++ );
}

void CArtsperrView::GenHeader( CString& cStr, CString cText, int nSeite, int nLng )
{
	CString s;
	long datum;
	long zeit;
...
bool CArtsperrView::GetInfoFromCallingView( CString& cStr, int nAction )
{
	...
	case 1:
		{
			...
			cText.Format( "NEM-gesperrte Artikel vom %s - %s", cDatVon, cDatBis );
...
void CArtsperrView::GotoDruckView(  )
{
...
void CArtsperrView::OnFilePrint() 
{
	// TODO: Add your command handler code here
	GotoDruckView(  );
}

void CArtsperrView::OnUpdateFilePrint(CCmdUI* pCmdUI) 
{
...
void CArtsperrView::OnFileSpeichern() 
{
...
void CArtsperrView::OnUpdateFileSpeichern(CCmdUI* pCmdUI) 
{
