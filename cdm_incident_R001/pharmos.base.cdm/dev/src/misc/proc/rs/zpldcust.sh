#! /usr/bin/ksh
 ...
 #Creation-Time: 00:00
 #Purpose:       starts, stops and status the binary zpldcust
 ###############################################################################################
 ...
 {
 	 if [[ "$2" == "" ]]; then set $1 "06.08.08 15:21:50"; fi
 ...
 function eval_parameters
 {
	 ...
	 #--------------------------#
 	 ...
 	 if [[ "$mode" == "" ]] || (( 1 != `check_string_parameter "$mode"` )); then
 		 printStartStop "$STARTMSG"
 		 rc=12;
 ...
 {
	 local readonly _PARA="$BRANCHNO_OPTION $branchno $MODE_OPTION $mode";
 ...
 if [[ "$start" == "YES" ]]; then
 	 ...
 	 rc=$?;
         if [[ $rc -ne 0 ]]
         then
            winmail -e "r.raab@phoenix-ag.de;" -c "f.naumann@phoenix-ag.de" -b "Abbruch zpldcust Fil. $branchno" -no -dos -nocc
         fi
