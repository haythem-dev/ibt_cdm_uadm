...
#Purpose:           Starts, stops and status the exemplar binary importmedreg for presentation only.            
###############################################################################################
...
readonly BINARY=importmedreg.bin
...
readonly REQ_NWEB_OPTION="-noweb"
readonly REQ_COMP_OPTION="-companies"
readonly REQ_MEDP_OPTION="-medicalpersons"
...
function eval_parameters
{
	 ...
	 do
		 ...
		 if [[ "$1" == "$TST_OPTION" ]]; then tst_env="YES"; sel_env="tst"; shift; continue; fi
		 if [[ "$1" == "$ACC_OPTION" ]]; then acc_env="YES"; sel_env="acc"; shift; continue; fi
		 if [[ "$1" == "$PRD_OPTION" ]]; then prd_env="YES"; sel_env="prd"; shift; continue; fi
		 if [[ "$1" == "$REQ_NWEB_OPTION" ]]; then request="no"; shift; continue; fi
		 if [[ "$1" == "$REQ_COMP_OPTION" ]]; then request="companies"; shift; continue; fi
		 if [[ "$1" == "$REQ_MEDP_OPTION" ]]; then request="personals"; shift; continue; fi
		break;
	 done
	 #--------------------------#
	 # evaluation of options
	 #--------------------------#
	 if [ ! "${start}${stop}${status}${send}${sel_env}${request}" ] ; then 
		 printStartStop "${STARTMSG}"
		 rc=10;
...
function assembling_process
{
	 ...
	 if [[ "$request" != "" ]]; then
		process="$process --load_medreg=$request"
	 else
		process="$process --load_medreg=no"
	 fi
}
###############################################################################################
...
if [[ "$sel_env" != "" ]]; then
	. $(dirname $0)/../vars/importmedreg_web_${sel_env}.vars;
fi
...
if [[ "$start" == "YES" ]]; then
	if [[ "$sel_env" != "" ]]; then
		...
		if [[ $rc == 3 ]]; then
			#do csv file mail
			$(dirname $0)/importmedreg_mail_data.sh -${sel_env} -${request};
			rc=0;
		else
			#do error mail
			$(dirname $0)/importmedreg_mail_err.sh -${sel_env} -${request};
		fi
	else
		echo "no environment option (-tst|-acc|-prd) defined for start" | tee -a ${LOG_RESOURCE}
	fi
fi
#======================================================#
