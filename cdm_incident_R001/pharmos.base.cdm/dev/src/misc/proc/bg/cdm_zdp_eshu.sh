 ...
 #Creation-Time:	15:05
 #Purpose:			starts, stops and status the exemplar binary zdpuiserv_bg for presentation only
 ###############################################################################################
 ...
 export CLIENT_LOCALE=bg_bg.1251
 ...
 readonly FORK_BINARY=zdpuiserv_bg.bin


 #--------------------------------------------------------------------#
 # INDIVIDUAL VARIABLES
 #--------------------------------------------------------------------#
 readonly PARAM_PORT=s:cdm_zdp_bg;
 start="";
 stop="";
 status="";
 ...
 function check_existence_of_binary
 {
 	 ...
 	 if [[ ! -f ${BIN_PATH}/${FORK_BINARY} ]]; then
 		 printStartStop "$STARTMSG"
 		 echo "Error: Binary \"${BIN_PATH}/${FORK_BINARY}\" does not exist!"
 		 printStartStop "$STOPMSG"
 		 exit 8;
 	 fi
}
 ...
 cd ${BIN_PATH}; # To enable the binary to work with the c-function 'getcwd()'
 ...
 if [[ "$status" == "YES" ]]; then
	 ...
	 msg=`status_process "${BIN_PATH}/${FORK_BINARY}"`
 	 typeset -i rc_fork=$?;
 	 echo "$msg" | tee -a $LOG_RESOURCE 
