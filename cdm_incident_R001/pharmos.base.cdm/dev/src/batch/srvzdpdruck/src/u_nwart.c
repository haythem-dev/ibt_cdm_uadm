...
int NrInSql( char *cSqlAnf, char *strNr, int anz )
{
    ...
    i = 0;
    j = 0;
    while ( i < anz && cSqlAnf[j] )
    {
        if ( memcmp( &cSqlAnf[j], strNr, lng ) == 0 )
            return i;
        while ( cSqlAnf[j] && cSqlAnf[j] != ',' ) j++;
        if ( cSqlAnf[j] )
            j++;
        i++;
    }
...
static long Lese_Protinfo( char *cNem, char *cSql, int *akt_anz, int max_anz, char *cErrmld )
{
    int  fd; 
    long s;
    long protokol_nr;
    ...
    *akt_anz = 0;
    ...
    if ( s != 0 )
    {
        ...
        SqlCloseCs( fd );
        if ( s == 100 )
            strcpy( cErrmld, "Keine Artikelaenderung im angegebenen Zeitraum" );
        return s;
    }
...
static long GetSelect( int *inr, int *akt_anz, char *cNem, char *sql, char *cErrmld )
{
    ...
    strcpy( sql, "select z.artikel_nr,z.einheit,z.artikel_name,z.nahrungsergaenzung,z.datum_aender," \
                 "z.hersteller_nr,name,z.zentr_eksperre,w.warengruppe " \
                 "from zartikel z, outer herstel h, outer zartwg w where z.artikel_nr in (" );
    ...
    if ( s == 100 )
    {
        if ( *akt_anz > 0 )
        {
            strcat( sql, ") and h.hersteller_nr=z.hersteller_nr and z.artikel_nr=w.artikel_nr and w.art = '1' order by artikel_name" );
...
long Lese_Protinfo_Art(  char *w_buf, int w_lng, char *r_buf, int *r_lng )
{
    long  s;
    int   akt_anz;
    int   max_anz;
