...
static long Lese_Protinfo( char *cNem, char *cSql, int *akt_anz, int max_anz, char *cErrmld )
{
    ...
    *akt_anz = 0;
    ...
    if ( s != 0 )
    {
        ...
        SqlCloseCs( fd );
        if ( s == 100 )
            strcpy( cErrmld, "Keine Lieferantenaenderung im angegebenen Zeitraum" );
        return s;
    }

    s = SqlGetTabDesc( "liefer", ( void ** ) &tab_desc, DESC_COLNAMES, cErrmld );
    if ( s != 0 )
    {
        SqlCloseCs( fd );
        return s;
        ...
        *buf_before = '\0';
        ...
        if ( s == 0 && protokol_nr == akt_protokol_nr )
        {
            ...
            *buf_after = '\0';
            ...
            {
                long nummer;
                nummer = (( struct LIEFER *) struc_before )->LANT_NR;
                if ( nummer == (( struct LIEFER *) struc_after )->LANT_NR ) 
                {
                ...
                {
                    s = 4713;
                    strcpy( cErrmld, "BufBefor, BufAfter: ungleiche Lieferanten-Nummern" );
                }
            } 
        } 
        else
...
long GetSelect( int *inr, int *akt_anz, char *cNem, char *sql, char *cErrmld )
{
    long s;

    strcpy( sql, "select lant_nr,lant_name,ort,nahrungsergaenzung,datum_aender " \
...
long Lese_Protinfo_Lie(  char *w_buf, int w_lng, char *r_buf, int *r_lng )
{
    long  s;
    int   akt_anz;
    int   max_anz;
