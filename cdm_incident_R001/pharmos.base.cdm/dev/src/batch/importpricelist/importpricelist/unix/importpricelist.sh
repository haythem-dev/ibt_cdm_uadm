#! /usr/bin/ksh
 ...
 #LIBRARY PATH 
 readonly LIBRARY_PATH=${LIB_PATH}/marathon/ssuk-01.13.02.00/sharedlib/release:${LIB_PATH}/tcaccess/8.0.11.4/sharedlib/release:${LIB_PATH}/boost/1.70.0_1/sharedlib:${LIB_PATH}/log4cplus/1.2.2_6/sharedlib
 export LD_LIBRARY_PATH=${LIBRARY_PATH}:${LD_LIBRARY_PATH}
 readonly BINARY=importpricelist.bin
 ...
 readonly INPUTPATH_OPTION="-inputpath";
 readonly INPUTFILE_OPTION="-inputfile";
 readonly PROCDATE_OPTION="-procdate";
 ...
 function print_local_var
 {
	 ...
	 printf "%-30s%-90s\n" "\$PROCDATE_OPTION:" ">$PROCDATE_OPTION<"
 ...
 function eval_parameters
 {
	 ...
	 if [[ "${procdate}" == "" ]] || (( 1 != `check_numeric_parameter "${PROCDATE_OPTION}" "${procdate}"` )); then
 		 printStartStop "${STARTMSG}"
 		 rc=12;
 		 echo "${PROCNAME}: Parameter value missed or invalid value \${procdate}: >${procdate}<!" | tee -a ${LOG_RESOURCE}
 		 echo ${USAGE} | tee -a ${LOG_RESOURCE}
 		 printStartStop "${STOPMSG}" "with Exit-Code: <${rc}>"
 		 exit ${rc};
 	 fi
	 
	 if [[ "${inputpath}" == "" ]] || (( 1 != `check_string_parameter "${INPUTPATH_OPTION}" "${inputpath}"` )); then
 		 printStartStop "${STARTMSG}"
 		 rc=12;
 		 echo "${PROCNAME}: Parameter value missed or invalid value \${inputpath}: >${inputpath}<!" | tee -a ${LOG_RESOURCE}
 		 echo ${USAGE} | tee -a ${LOG_RESOURCE}
 		 printStartStop "${STOPMSG}" "with Exit-Code: <${rc}>"
 		 exit ${rc};
 	 fi
 ...
 function assembling_process
 {
	 ...
	 if (( ${#_parameters} > 0 )); then
 		 _parameters="${_parameters} $INPUTFILE_OPTION $inputpath/$inputfile"
 	 else
 		 _parameters="$INPUTFILE_OPTION $inputfile";
 	 fi

	 if (( ${#_parameters} > 0 )); then
 		 _parameters="${_parameters} $PROCDATE_OPTION $procdate"
 	 else
 		 _parameters="$PROCDATE_OPTION $procdate";
 	 fi
 ...
 ###############################################################################################
 . $(dirname $0)/importpricelist.option
