#! /usr/bin/ksh
 ...
 readonly DATE_OPTION="-calcDate";
 readonly TIME_OPTION="-calcTime";
 ...
 function assembling_process
 {
	 local _parameters="";
 	 ...
 	 if (( ${#_parameters} > 0 )); then
         ...
         if (( $calcDate > 0 )); then
            if (( $calcTime > 0 )); then
               _parameters="${_parameters} $DATE_OPTION $calcDate $TIME_OPTION $calcTime"
            fi
         fi
 ...
 ###############################################################################################
 . $(dirname $0)/subsequentdeliveryrelevantcalculator.option
