...
set -x
 ...
 dbaccess $CDM_DB << end
   unload to ../data/csubsequentdeliveryrule.csv
     delimiter ';'
     select * from csubsequentdeliveryrule
       order by ruleid;

   unload to ../data/csubsequentdeliveryparam.csv
     delimiter ';'
     select * from csubsequentdeliveryparam
       order by branchno, ruleid;
end
 exit ${rc};
