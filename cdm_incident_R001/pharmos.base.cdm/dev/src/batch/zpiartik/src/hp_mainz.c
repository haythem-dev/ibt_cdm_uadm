...
static char ret_buf_klinik[500];
...
void main ( int argc, char *argv[] )
{
   ...
   HNMP handle;
      ...
      {
         if ( argc > 1 )
            s = ProgAuswahl( argc - 1, &argv[1], ret_buf, ret_buf_klinik );
         else
         {
            s = 1;
            ProgUsage( ret_buf );
...
int main ( int argc, char *argv[] )
{
    ...
    if ( anz_arg > 1 )
    {
	    ...
	    s = ProgAuswahl( anz_arg - 1, &argv[1], ret_buf, ret_buf_klinik );
    }
    else
    {
        s = 1;
