...
#define READ_ANZ 8000  /* angepasst an Pipe-Größe (siehe libscsvoodoo/udupipe.h) - KT 19.10.21 */

static char ret_buf[READ_ANZ+1];
...
void main ( int argc, char *argv[] )
{
    ...
    HNMP handle;
      ...
      if ( ( s = ViaEshuOpen( &handle, "zdev21", "uiserv" ) ) == 0 )
      {
            *ret_buf = '\0';
            if ( argc > 1 )
...
/* Beginn Kopierbereich aus zpautidem */
...
{
   if( *str )
      write_logfile( status, 0, program, str );
}
...
/* Ende Kopierbereich aus zpautidem */
...
void main ( int argc, char *argv[] )
{
    ...
    *ret_buf = '\0';
    lng = PipwRead ( argc, argv, ret_buf, READ_ANZ );   /* pipe lesen */
    if ( lng == -1 )
    {
