...
readonly BINARY=export-data-with-current-aep.sh
...
readonly INPUT_OPTION="-inputcsv";
readonly FTPHOST_OPTION="-host";
readonly FTPUSER_OPTION="-user";
readonly FTPPASS_OPTION="-pass";
readonly FTPFOLDER_OPTION="-folder";
...
function eval_parameters
{
	 ...
	 if (( ${options_counter} > 1 )) ; then
 	 ...
 	 if [ ! "${ftphost}${ftpuser}${ftppass}${ftpfolder}" ] ; then
		 printStartStop "${STARTMSG}" 
		 rc=13; 
		 echo "${PROCNAME}: ftp host, user, pass and folder missing!" | tee -a ${LOG_RESOURCE} 
		 echo ${USAGE} | tee -a ${LOG_RESOURCE} 
		 printStartStop "${STOPMSG}" "with Exit-Code: <${rc}>" 
		 exit ${rc}; 
         fi
...
function assembling_process
{
	 ...
	 _parameters="${_parameters} -h ${ftphost} -u ${ftpuser} -p ${ftppass} -l ${ftpfolder}";
 	 if (( ${#_parameters} > 0 )); then
...
###############################################################################################
. $(dirname $0)/export-data-with-current-aep.option
