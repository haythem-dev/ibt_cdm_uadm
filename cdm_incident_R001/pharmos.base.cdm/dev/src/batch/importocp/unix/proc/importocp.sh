	 ...
	 if (( ${options_counter} > 1 )) ; then
 	 ...
 	 if [[ "${input}" == "YES" ]]; then 
 		 if [[ "${inputfile}" == "" ]]; then 
 			 printStartStop "${STARTMSG}" 
 			 rc=12; 
 			 echo "${PROCNAME}: inputfile missed or invalid value \${inputfile}: >${inputfile}<!" | tee -a ${LOG_RESOURCE} 
 			 echo ${USAGE} | tee -a ${LOG_RESOURCE} 
 			 printStartStop "${STOPMSG}" "with Exit-Code: <${rc}>" 
 			 exit ${rc}; 
 		 fi 
		 ...
		 printStartStop "${STARTMSG}"
		 rc=10;
		 echo "${PROCNAME}: No inputcsv set in command line!" | tee -a ${LOG_RESOURCE}
		 echo ${USAGE} | tee -a ${LOG_RESOURCE}
		 printStartStop "${STOPMSG}" "with Exit-Code: <${rc}>"
		 exit ${rc};
         fi
...
function assembling_process
{
	 ...
	 _parameters="${_parameters} ${INPUT_OPTION} ${inputfile}";
 	 if (( ${#_parameters} > 0 )); then
...
if [[ "$start" == "YES" ]]; then
	...
	if [[ $rc != 0 ]]; then
		#do error mail
		${PROC_PATH}/importocp_mail_err.sh;
 	fi
	#do missing mail
