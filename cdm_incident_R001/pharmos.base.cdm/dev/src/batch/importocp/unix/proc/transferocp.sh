...
readonly OUTPUT_OPTION="-outpharmos";
...
function eval_parameters
{
	 ...
	 do
		 ...
		 if [[ "$1" == "$OUTPUT_OPTION" ]]; then
			 ...
			 if (( $# > 0 )); then 
				 outputdate=$1;
				 # workaround to fix in case the hour is given with one digit only
				 len=${#outputdate}
				 if [[ $len == 13 ]]; then
					xdate=${outputdate%${outputdate#????????}}
					xtime="${outputdate#*${xdate}}"
					outputdate="${xdate}0${xtime}"
					echo "outputdate too short (len=$len), fixed it to ${outputdate}"
				 fi
	 ...
	 if (( ${options_counter} > 1 )) ; then
 	 ...
 	 if [[ "${output}" == "YES" ]]; then 
 		 if [[ "${outputdate}" == "" ]]; then 
 			 printStartStop "${STARTMSG}" 
 			 rc=12; 
 			 echo "${PROCNAME}: outputdate missed or invalid value \${outputdate}: >${outputdate}<!" | tee -a ${LOG_RESOURCE} 
 			 echo ${USAGE} | tee -a ${LOG_RESOURCE} 
 			 printStartStop "${STOPMSG}" "with Exit-Code: <${rc}>" 
 			 exit ${rc}; 
 		 fi 
		 ...
		 printStartStop "${STARTMSG}"
		 rc=10;
		 echo "${PROCNAME}: No outpharmos set in command line!" | tee -a ${LOG_RESOURCE}
		 echo ${USAGE} | tee -a ${LOG_RESOURCE}
		 printStartStop "${STOPMSG}" "with Exit-Code: <${rc}>"
		 exit ${rc};
         fi
...
function assembling_process
{
	 ...
	 _parameters="${_parameters} ${OUTPUT_OPTION} ${outputdate}";
	 if [[ "${username}" != "" ]]; then
