...
readonly INPUTFILE_OPTION="-inputcsv";
readonly INPUTDATETIME_OPTION="-datetime"
...
function eval_parameters
{
	 ...
	 do
		 ...
		 if [[ "$1" == "$INPUTDATETIME_OPTION" ]]; then
			 shift;
			 if (( $# > 0 )); then 
				 inputdatetime=$1;
				 shift; 
	 ...
	 if (( ${options_counter} > 1 )) ; then
 	 ...
 	 if [[ "${inputfile}" == "" ]] || [[ "${inputdatetime}" == "" ]]; then 
		 printStartStop "${STARTMSG}" 
		 rc=12; 
		 echo "${PROCNAME}: input params missed or invalid value ${INPUTFILE_OPTION}: >${inputfile}<, ${INPUTDATETIME_OPTION}: >${inputdatetime}<!" | tee -a ${LOG_RESOURCE} 
		 echo ${USAGE} | tee -a ${LOG_RESOURCE} 
		 printStartStop "${STOPMSG}" "with Exit-Code: <${rc}>" 
		 exit ${rc}; 
	 fi 
...
function assembling_process
{
	 ...
	 _parameters="${_parameters} ${INPUTFILE_OPTION} ${inputfile} -inexdb ${INPUTDATETIME_OPTION} ${inputdatetime}";
 	 if (( ${#_parameters} > 0 )); then
...
if [[ "$start" == "YES" ]]; then
	...
	#do missing mail
	${PROC_PATH}/importocp_mail_missing.sh;
fi
#======================================================#
