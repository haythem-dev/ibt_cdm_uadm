...
#Purpose:           Starts, stops and status the exemplar binary importocp for presentation only.            
###############################################################################################
...
#LIBRARY PATH 
readonly LIBRARY_PATH=${LIB_PATH}/stlport/4.5.3/sharedlib/release
export LD_LIBRARY_PATH=${LIBRARY_PATH}:${LD_LIBRARY_PATH}
readonly BINARY=importocp.sh
...
function eval_parameters
{
	 ...
	 if (( ${options_counter} > 1 )) ; then
 	 ...
 	 if [ ! "${inputpath}" ]; then 
		 printStartStop "${STARTMSG}" 
 		 rc=13; 
 		 echo "${PROCNAME}: inputpath missed!" | tee -a ${LOG_RESOURCE} 
 		 echo ${USAGE} | tee -a ${LOG_RESOURCE} 
 		 printStartStop "${STOPMSG}" "with Exit-Code: <${rc}>" 
 		 exit ${rc}; 
 	 fi 
...
#======================================================#
if [[ "$start" == "YES" ]]; then
	echo "OCP article file transfer process start" | tee -a ${LOG_RESOURCE}
	cd $inputpath
	for file in $( ls -rtl *ArtMaster_03_00.txt | awk '{print $9}' )
	do
		echo "move data file from PINT ${inputpath}/${file}..." | tee -a ${LOG_RESOURCE}
        	mv "$file" "${DATA_PATH}/ocp/${file}"
		echo "import data file from DATA ${DATA_PATH}/ocp/${file}..." | tee -a ${LOG_RESOURCE}
		${PROC_PATH}/importocp.sh -start -inputcsv ${file}
		rc=$?;
		if [[ $rc == 0 ]]; then
			FILENAMENOEXT="${file%.*}"
			echo "import done, so rename file" | tee -a ${LOG_RESOURCE}
			mv "${DATA_PATH}/ocp/${file}" "${DATA_PATH}/ocp/${FILENAMENOEXT}.txt_imported"
		fi
	done;
	echo "OCP article file transfer process end" | tee -a ${LOG_RESOURCE}
	rc=0
fi
#======================================================#
