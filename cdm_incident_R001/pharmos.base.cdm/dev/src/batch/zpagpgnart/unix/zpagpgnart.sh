#! /usr/bin/ksh
 ...
 #Purpose:           Starts, stops and status the exemplar binary zpagpgnart for presentation only.            
 ###############################################################################################
 ...
 function eval_parameters
 {
	 ...
	 #--------------------------#
 	 ...
 	 #--------------------------#
 	 if (( ${import} > 0 )); then
 		 if [[ "${import}" == "" ]] || (( 1 != `check_numeric_parameter "${IMPORT_OPTION}" "${import}"` )) || [[ "${import}" == "0" ]]; then
 			 printStartStop "${STARTMSG}"
 			 rc=12;
 			 echo "${PROCNAME}: Parameter value missed or invalid value \${import}: >${import}<!" | tee -a ${LOG_RESOURCE}
 			 echo ${USAGE} | tee -a ${LOG_RESOURCE}
 			 printStartStop "${STOPMSG}" "with Exit-Code: <${rc}>"
 			 exit ${rc};
 		 fi
 ...
 function assembling_process
 {
	 local _parameters="";
 	 if (( $import > 0 )); then
 		 _parameters="${_parameters} $IMPORT_OPTION ";
 		 _parameters="${_parameters} $import ";
 	 fi
 ...
 . $(dirname $0)/../vars/zpagpgnart.vars;
