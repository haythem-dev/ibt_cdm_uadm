...
#Purpose:           Starts, stops and status the exemplar binary zpconsig for presentation only.            
###############################################################################################
...
readonly PROCDATE_OPTION="-date";
readonly SIM_OPTION="-sim";
...
function assembling_process
{
	 ...
	 if [ "${sim}" ] ; then 
 	 	if (( ${#_parameters} > 0 )); then
 			 _parameters="${_parameters} $SIM_OPTION $sim";
 	 	else
 			 _parameters="$SIM_OPTION $sim";
 	 	fi
	 fi
