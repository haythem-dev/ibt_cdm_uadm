#! /usr/bin/ksh
 ...
 readonly BINARY=exportarticlecalculator.bin
 ...
 function eval_parameters
 {
	 ...
	 if [[ "${datefrom}" == "" ]] || (( 1 != `check_numeric_parameter "${DATEFROM_OPTION}" "${datefrom}"` )); then
 		 printStartStop "${STARTMSG}"
 		 rc=12;
 ...
 function assembling_process
 {
	 ...
	 if (( ${#_parameters} > 0 )); then
 		 _parameters="${_parameters} $DATEFROM_OPTION $datefrom"
 	 else
 		 _parameters="$DATEFROM_OPTION $datefrom";
 	 fi
 ...
 ###############################################################################################
 . $(dirname $0)/exportarticlecalculator.option
 ...
 assembling_process;
 print_msg "$STARTMSG" "$*"
 export EXPORTARTICLECALC_BRANCHNO=$branchno
 export EXPORTARTICLECALC_COUNTRY=$country
 export EXPORTARTICLECALC_DATE=`date +%Y-%m-%d`
