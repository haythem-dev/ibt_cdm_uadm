...
void process_transaction(const char* dbname, ini_file_t* ini, int txn_idx) {
    ...
    for (int iter = 0; iter < repeat_count; ++iter) {
        ...
        clock_t start = clock();
        int result = dbtest_transaction(dbname, expanded_sql, ini, section);
        clock_t end = clock();
...
int main(int argc, char* argv[]) {
    ...
    for (int txn_idx = 0; ; ++txn_idx) {
        ...
        for (int iter = 0; iter < txn.repeat_count; ++iter) {
            ...
            clock_t start = clock();
            int result = dbtest_transaction(dbname, txn.expanded_sql, ini, txn.section);
            clock_t end = clock();
            double elapsed = (double)(end - start) / CLOCKS_PER_SEC;
            total_time += elapsed;
            if (txn.has_iteration) {
