...
-- =============================================================================
-- MULTI-BRANCH ARCHIVE PREDICTION QUERIES - AUSTRIA (AT)
-- Based on actual tpldauftr.cxx script logic analysis
-- Extended to cover all AT branches with per-branch results
-- =============================================================================
-- AT Branch List: 61,62,63,64,99
-- Cutoff Date: 20250815 (example)
-- Analysis Date: January 16, 2025
-- =============================================================================

-- =============================================================================
-- SCENARIO 1: STANDARD MODE (No H24 flag) - AT BRANCHES
-- Command: ./tpldauftr.sh -start -fil <branch> -del 30
-- Logic: Date-based archiving with standard status validation
-- =============================================================================

SELECT 
    'STANDARD MODE AT' as execution_mode,
    'Date-based archiving (no H24 flags) - Austria' as description,
    filialnr as branch_number,
    
    -- Branch name lookup for Austria
    CASE filialnr
        WHEN 61 THEN '--- ZDEV --- WIEN --- ZDEV ---'
        WHEN 62 THEN '--- ZDEV --- GRAZ --- ZDEV ---'
        WHEN 63 THEN '--- ZDEV --- LINZ --- ZDEV ---'
        WHEN 64 THEN '--- ZDEV --- HALL --- ZDEV ---'
        WHEN 99 THEN 'ZDEV21 Austria'
        ELSE 'Unknown AT Branch'
    END as branch_name,
...
-- =============================================================================
-- SCENARIO 2: H24 MODE (Status-flag-based archiving) - AT BRANCHES
-- Command: ./tpldauftr.sh -start -fil <branch> -del 30 -h24
-- Logic: H24 flags override date logic for immediate archiving
-- =============================================================================

SELECT 
    'H24 MODE AT' as execution_mode,
    'Status-flag-based archiving (H24 flags active) - Austria' as description,
    filialnr as branch_number,
...
-- =============================================================================
-- COMPREHENSIVE SUMMARY QUERY - AT BRANCHES
-- Compare all execution modes side by side per branch
-- =============================================================================

SELECT 
    'EXECUTION MODE COMPARISON AT' as analysis_type,
    filialnr as branch_number,
...
FROM (SELECT DISTINCT filialnr FROM kdauftrag 
      WHERE filialnr IN (61,62,63,64,99)) k
ORDER BY filialnr;
