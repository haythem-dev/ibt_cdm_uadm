#! /usr/bin/ksh
 ...
 #Creation-Time:     17:27                                                                                     
 #Purpose:           Starts, stops and status the exemplar binary delkdkritik for presentation only.           
 ###############################################################################################
 ...
 #--------------------------------------------------------------------#
 readonly BINARY=delkdkritik.bin
 ...
 typeset -i delolderthan=0;
 typeset -i callbackstatus=0;
 typeset -i iscallbackoptiongiven=0;
 ...
 readonly DELOLDERTHAN_OPTION="-delolderthan";
 readonly CALLBACKSTATUS_OPTION="-callbackstatus";
 ...
 function print_local_var
 {
	 ...
	 printf "%-30s%-90s\n" "\$DELOLDERTHAN_OPTION:" ">$DELOLDERTHAN_OPTION<"
 ...
 function eval_parameters
 {
 	 ...
 	 do
         ...
         if [[ "$1" == "$CALLBACKSTATUS_OPTION" ]]; then
 			 shift;
 			 if (( $# > 0 )) ; then
 				 callbackstatus=$1;
                                 iscallbackoptiongiven=1;
 				 shift;
 				 continue;
 			 else 
 				 break;
	 ...
	 #--------------------------#
 	 ...
 	 #--------------------------#
 	 if [[ "${delolderthan}" == "" ]] || (( 1 != `check_numeric_parameter "${DELOLDERTHAN_OPTION}" "${delolderthan}"` )) || [[ "${delolderthan}" == "0" ]]; then
 		 printStartStop "${STARTMSG}"
 		 rc=12;
 		 echo "${PROCNAME}: Parameter value missed or invalid value \${delolderthan}: >${delolderthan}<!" | tee -a ${LOG_RESOURCE}
 		 echo ${USAGE} | tee -a ${LOG_RESOURCE}
 		 printStartStop "${STOPMSG}" "with Exit-Code: <${rc}>"
 		 exit ${rc};
 	 fi
     if (( ${iscallbackoptiongiven} == 1 )) ; then
         if [[ "${callbackstatus}" == "" ]] || (( 1 != `check_numeric_parameter "${CALLBACKSTATUS_OPTION}" "${callbackstatus}"` )) || [[ "${callbackstatus}" == "0" ]]; then
         printStartStop "${STARTMSG}"
 		 rc=12;
 		 echo "${PROCNAME}: Parameter value missed or invalid value \${callbackstatus}: >${callbackstatus}<!" | tee -a ${LOG_RESOURCE}
 		 echo ${USAGE} | tee -a ${LOG_RESOURCE}
 		 printStartStop "${STOPMSG}" "with Exit-Code: <${rc}>"
 		 exit ${rc};
         fi
 ...
 function check_numeric_parameter
 {
 	 ...
 	 if [[ "${_OPTION}" == "${DELOLDERTHAN_OPTION}" ]]; then
 ...
 function assembling_process
 {
	 local _parameters="";
 	 ...
 	 if (( ${#_parameters} > 0 )); then
 		 _parameters="${_parameters} $DELOLDERTHAN_OPTION $delolderthan"
 	 else
 		 _parameters="$DELOLDERTHAN_OPTION $delolderthan";
 	 fi
     ...
     if (( ${iscallbackoptiongiven} == 1 )) ; then
         process="$process -callbackstatus ${callbackstatus}"
     fi
 ...
 . $(dirname $0)/../vars/delkdkritik.vars;
