#! /usr/bin/ksh
 ...
 #LIBRARY PATH 
 readonly LIBRARY_PATH=${LIB_PATH}/marathon/ssuk-01.05.02.00/sharedlib/release:${LIB_PATH}/tcaccess/8.0.0.7/sharedlib/release
 ...
 {
 	 if [[ "$2" == "" ]]; then set $1 "19.11.08 12:15:54"; fi
 ...
 function print_local_var
 {
	 ...
	 echo "\$BRANCH_OPTION:			>$BRANCH_OPTION<"
	 echo "\$T_OPTION:			>$T_OPTION<"
	 echo "\$SO_OPTION:			>$SO_OPTION<"
	 echo "\$DO_OPTION:			>$DO_OPTION<"
 ...
 function eval_parameters
 {
	 ...
	 #--------------------------#
 	 ...
 	 #--------------------------#
 	 if [[ "$branch" == "" ]] || (( 1 != `check_numeric_parameter "$branch"` )) || [[ "$branch" == "0" ]]; then
 		 printStartStop "$STARTMSG"
 		 rc=12;
 	 ...
 	 if [[ "$t" == "" ]] || (( 1 != `check_string_parameter "$t"` )); then
 		 printStartStop "$STARTMSG"
 		 rc=12;
 	 ...
 	 if [[ "$so" == "" ]] || (( 1 != `check_string_parameter "$so"` )); then
 		 printStartStop "$STARTMSG"
 		 rc=12;
 	 ...
 	 if [[ "$do" == "" ]] || (( 1 != `check_string_parameter "$do"` )); then
 		 printStartStop "$STARTMSG"
 		 rc=12;
 ...
 function assembling_process
 {
	 local _parameters="";
 	 ...
 	 if (( ${#_parameters} > 0 )); then
 		 _parameters="${_parameters} $BRANCH_OPTION $branch $T_OPTION $t $SO_OPTION $so $DO_OPTION $do"
 	 else
 		 _parameters="$BRANCH_OPTION $branch $T_OPTION $t $SO_OPTION $so $DO_OPTION $do";
 	 fi
 ...
 . $(dirname $0)/../vars/cscozek.vars;

 check_existence_of_binary;
 assembling_process;
 print_msg "$STARTMSG" "$*"
 export CSCOZEK_BRANCHNO=$branchno
