#! /usr/bin/ksh
 ...
 #Creation-Time:     17:01                                                                                     
 #Creator:           procgen_in_dev.sh - Version: 01.01.00.00                                                  
 #Creation-Type:     ONLINE_SERVER_PROCEDURE                                                                   
 #Purpose:           Starts, stops and status the exemplar binary unload_debtrel for presentation only.        
 ###############################################################################################
 ...
 #--------------------------------------------------------------------#
 readonly BINARY=unload_debtrel.bin
 ...
 function eval_parameters
 {
	 ...
	 #--------------------------#
 	 ...
 	 if [[ "${date}" == "" ]] || (( 1 != `check_numeric_parameter "${DATE_OPTION}" "${date}"` )) || [[ "${date}" == "0" ]]; then
 		 printStartStop "${STARTMSG}"
 		 rc=12;
 		 echo "${PROCNAME}: Parameter value missed or invalid value \${date}: >${date}<!" | tee -a ${LOG_RESOURCE}
 		 echo ${USAGE} | tee -a ${LOG_RESOURCE}
 		 printStartStop "${STOPMSG}" "with Exit-Code: <${rc}>"
 		 exit ${rc};
 	 fi
 ...
 function assembling_process
 {
	 local _parameters="";
 	 ...
 	 if (( ${#_parameters} > 0 )); then
 		 _parameters="${_parameters} $COUNTRY_OPTION $country $DB_OPTION $db $DATE_OPTION $date"
 	 else
 		 _parameters="$COUNTRY_OPTION $country $DB_OPTION $db $DATE_OPTION $date";
 	 fi
 ...
 ###############################################################################################
 . $(dirname $0)/unload_debtrel.option
