#! /usr/bin/ksh
 ...
 #Purpose:           Starts, stops and status the exemplar binary longtermlackloader for presentation only.    
 ###############################################################################################
 ...
 readonly BINARY=longtermlackloader.bin
 ...
 function eval_parameters
 {
	 ...
	 #--------------------------#
 	 ...
 	 #--------------------------#
 	 if [[ "${areaid}" == "" ]] || (( 1 != `check_numeric_parameter "${AREAID_OPTION}" "${areaid}"` )) || [[ "${areaid}" == "0" ]]; then
 		 printStartStop "${STARTMSG}"
 		 rc=12;
 		 echo "${PROCNAME}: Parameter value missed or invalid value \${areaid}: >${areaid}<!" | tee -a ${LOG_RESOURCE}
 		 echo ${USAGE} | tee -a ${LOG_RESOURCE}
 		 printStartStop "${STOPMSG}" "with Exit-Code: <${rc}>"
 		 exit ${rc};
 	 fi
 ...
 function assembling_process
 {
	 local _parameters="";
 	 ...
 	 if (( ${#_parameters} > 0 )); then
 		 _parameters="${_parameters} $AREAID_OPTION $areaid"
 	 else
 		 _parameters="$AREAID_OPTION $areaid";
 	 fi
 ...
 ###############################################################################################
 . $(dirname $0)/longtermlackloader.option
 ...
 . $(dirname $0)/../vars/longtermlackloader.vars;
