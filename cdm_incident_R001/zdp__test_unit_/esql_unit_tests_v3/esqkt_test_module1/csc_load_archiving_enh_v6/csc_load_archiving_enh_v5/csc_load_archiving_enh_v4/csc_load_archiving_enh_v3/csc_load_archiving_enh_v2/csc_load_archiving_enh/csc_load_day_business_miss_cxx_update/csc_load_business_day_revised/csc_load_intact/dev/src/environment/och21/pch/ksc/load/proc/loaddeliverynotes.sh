#! /usr/bin/ksh
 ...
 #Creation-Time:	16:21
 #Purpose:			starts, stops and status the exemplar binary loaddeliverynotes for presentation only
 ###############################################################################################
 ...
 #--------------------------------------------------------------------#
 readonly BINARY=loaddeliverynotes.bin
 ...
 {
 	 if [[ "$2" == "" ]]; then set $1 "14.10.08 16:21:52"; fi
 ...
 function print_local_var
 {
	 ...
	 echo "\$INPUTFILE_OPTION:			>$INPUTFILE_OPTION<"
 ...
 function eval_parameters
 {
	 ...
	 #--------------------------#
 	 ...
 	 #--------------------------#
 	 if [[ "$inputfile" == "" ]] || (( 1 != `check_string_parameter "$inputfile"` )); then
 		 printStartStop "$STARTMSG"
 		 rc=12;
 ...
 function assembling_process
 {
	 local _parameters="";
 	 ...
 	 if (( ${#_parameters} > 0 )); then
 		 _parameters="${_parameters} $inputfile $databasename"
 	 else
 		 _parameters="$inputfile $databasename";
 	 fi
 ...
 ###############################################################################################
 . $(dirname $0)/loaddeliverynotes.option
