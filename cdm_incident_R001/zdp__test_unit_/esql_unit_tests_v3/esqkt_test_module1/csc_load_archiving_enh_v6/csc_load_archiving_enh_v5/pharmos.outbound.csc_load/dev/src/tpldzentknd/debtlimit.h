...
/******************************************************************************/
/* d:\prj\DF\UNIX\TPLDZENTKND\src\debtlimit ***********************************/
/******************************************************************************/
...
class CDEBTLIMIT : public ppDadeVirtual
{
	...
	public:
		...
		int CloseCursor(int pos = 0)  //close db cursor
		{
			...
			rc = ::ServerNr(pos, FncNumber, (void *)&s,DEBTLIMIT_BES,DEBTLIMIT_ANZ, error_msg);
			#else
...
/* CreateSqlStatement of DEBTLIMIT ********************************************/
 ...
 #define DEBTLIMIT_CREATE(x) exec sql create table x (\
         ...
         SUM_GOODSRECEIPT decimal(13,2) default 0 not null) in ksc extent size 8 next size 8 lock mode row;
