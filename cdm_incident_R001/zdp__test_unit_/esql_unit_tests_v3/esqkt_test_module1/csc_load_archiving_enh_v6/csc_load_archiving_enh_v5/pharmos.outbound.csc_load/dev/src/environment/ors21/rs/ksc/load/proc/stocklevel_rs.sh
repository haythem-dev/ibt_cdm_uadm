#! /usr/bin/ksh
 ...
 #Purpose:           Starts, stops and status the exemplar binary stocklevel_rs for presentation only.         
 ###############################################################################################
 ...
 #--------------------------------------------------------------------#
 readonly BINARY=stocklevel_rs._sh
 ...
 function assembling_process
 {
	 local _parameters="";
 	 ...
 	 if (( ${#_parameters} > 0 )); then
 		 _parameters="${_parameters} $CF_OPTION $cf $KSCDB_OPTION $kscdb $M_OPTION $m"
 	 else
 		 _parameters="$CF_OPTION $cf $KSCDB_OPTION $kscdb $M_OPTION $m";
 	 fi
 ...
 ###############################################################################################
 . $(dirname $0)/stocklevel_rs.option
