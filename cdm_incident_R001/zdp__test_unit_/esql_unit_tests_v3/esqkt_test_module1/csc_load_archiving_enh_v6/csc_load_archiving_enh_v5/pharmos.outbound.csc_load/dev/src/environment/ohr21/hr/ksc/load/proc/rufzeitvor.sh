#! /usr/bin/ksh
 ...
 #Creation-Time:	16:34
 #Purpose:			starts, stops and status the exemplar binary rufzeitvor for presentation only
 ###############################################################################################
 ...
 #--------------------------------------------------------------------#
 readonly BINARY=rufzeitvor.bin
 ...
 readonly UHRZEIT_OPTION="-uhrzeit";
 readonly VORZEIT_OPTION="-vorzeit";
 ...
 {
 	 if [[ "$2" == "" ]]; then set $1 "05.09.08 16:34:10"; fi
 ...
 function print_local_var
 {
	 ...
	 echo "\$UHRZEIT_OPTION:			>$UHRZEIT_OPTION<"
	 echo "\$VORZEIT_OPTION:			>$VORZEIT_OPTION<"
 ...
 function eval_parameters
 {
	 ...
	 #--------------------------#
 	 ...
 	 if [[ "$uhrzeit" == "" ]] || (( 1 != `check_numeric_parameter "$uhrzeit"` )) || [[ "$uhrzeit" == "0" ]]; then
 		 printStartStop "$STARTMSG"
 		 rc=12;
 		 echo "$PROCNAME: Parameter value missed or invalid value \$uhrzeit: >$uhrzeit<!" | tee -a $LOG_RESOURCE
 		 echo ${USAGE} | tee -a $LOG_RESOURCE
 		 printStartStop "$STOPMSG" "with Exit-Code: <${rc}>"
 		 exit ${rc};
 	 fi
 	 ...
 	 if [[ "$vorzeit" == "" ]] || (( 1 != `check_numeric_parameter "$vorzeit"` )) || [[ "$vorzeit" == "0" ]]; then
 		 printStartStop "$STARTMSG"
 		 rc=12;
 		 echo "$PROCNAME: Parameter value missed or invalid value \$vorzeit: >$vorzeit<!" | tee -a $LOG_RESOURCE
 		 echo ${USAGE} | tee -a $LOG_RESOURCE
 		 printStartStop "$STOPMSG" "with Exit-Code: <${rc}>"
 		 exit ${rc};
 	 fi
 ...
 function assembling_process
 {
	 local _parameters="";
 	 ...
 	 if (( ${#_parameters} > 0 )); then
 		 _parameters="${_parameters} $VZ_OPTION $vz $UHRZEIT_OPTION $uhrzeit $VORZEIT_OPTION $vorzeit"
 	 else
 		 _parameters="$VZ_OPTION $vz $UHRZEIT_OPTION $uhrzeit $VORZEIT_OPTION $vorzeit";
 	 fi
 ...
 . $(dirname $0)/../vars/rufzeitvor.vars;
