#! /usr/bin/ksh
 ...
 #Creation-Time:	14:23
 #Purpose:			starts, stops and status the exemplar binary tpldreakauft for presentation only
 ###############################################################################################
 ...
 #--------------------------------------------------------------------#
 readonly BINARY=tpldreakauft.bin
 ...
 readonly ERRDATE_OPTION="-errdate";
 ...
 {
 	 if [[ "$2" == "" ]]; then set $1 "10.09.08 14:23:10"; fi
 ...
 function print_local_var
 {
	 ...
	 echo "\$ERRDATE_OPTION:			>$ERRDATE_OPTION<"
 ...
 function eval_parameters
 {
	 ...
	 #--------------------------#
 	 ...
 	 #--------------------------#
 	 if (( ${errdate} > 0 )); then
 		 if [[ "$errdate" == "" ]] || (( 1 != `check_numeric_parameter "$errdate"` )) || [[ "$errdate" == "0" ]]; then
 			 printStartStop "$STARTMSG"
 			 rc=12;
 			 echo "$PROCNAME: Parameter value missed or invalid value \$errdate: >$errdate<!" | tee -a $LOG_RESOURCE
 			 echo ${USAGE} | tee -a $LOG_RESOURCE
 			 printStartStop "$STOPMSG" "with Exit-Code: <${rc}>"
 			 exit ${rc};
 		 fi
 ...
 function assembling_process
 {
	 local _parameters="";
 	 if (( $errdate > 0 )); then
 		 _parameters="${_parameters} $ERRDATE_OPTION ";
 		 _parameters="${_parameters} $errdate ";
 	 fi
 ...
 . $(dirname $0)/../vars/tpldreakauft.vars;
