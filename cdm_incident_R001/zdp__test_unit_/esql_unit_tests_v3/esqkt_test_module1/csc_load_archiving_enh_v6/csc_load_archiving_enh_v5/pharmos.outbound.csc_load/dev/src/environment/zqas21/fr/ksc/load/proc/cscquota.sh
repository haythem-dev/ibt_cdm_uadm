#! /usr/bin/ksh
 ...
 #Creation-Time:     12:46                                                                                     
 #Purpose:           Starts, stops and status the exemplar binary cscquota for presentation only.              
 ###############################################################################################
 ...
 #LIBRARY PATH 
 readonly LIBRARY_PATH=${LIB_PATH}/marathon/ssuk-01.09.00.00/sharedlib/release:${LIB_PATH}/tcaccess/8.0.11.4/sharedlib/release
 export LD_LIBRARY_PATH=${LIBRARY_PATH}:${LD_LIBRARY_PATH}
 readonly BINARY=cscquota.bin
 ...
 readonly PHARMACYGROUPID_OPTION="-pharmacygroupid";
 readonly CUSTOMERBASED_OPTION="-customerbased";
 readonly DBSERVER_OPTION="-dbserver";
 readonly DBNAME_OPTION="-dbname";
 readonly QUOTATIONFILE_OPTION="-quotationfile";
 ...
 function print_local_var
 {
	 ...
	 printf "%-30s%-90s\n" "\$PHARMACYGROUPID_OPTION:" ">$PHARMACYGROUPID_OPTION<"
	 printf "%-30s%-90s\n" "\$CUSTOMERBASED_OPTION:" ">$CUSTOMERBASED_OPTION<"
	 printf "%-30s%-90s\n" "\$DBSERVER_OPTION:" ">$DBSERVER_OPTION<"
	 printf "%-30s%-90s\n" "\$DBNAME_OPTION:" ">$DBNAME_OPTION<"
	 printf "%-30s%-90s\n" "\$QUOTATIONFILE_OPTION:" ">$QUOTATIONFILE_OPTION<"
 ...
 function eval_parameters
 {
 	 ...
 	 do
 		 ...
 		 #read in parameters
 		 if [[ "$1" == "$PHARMACYGROUPID_OPTION" ]]; then
 			 shift;
 			 if (( $# > 0 )) ; then
 				 pharmacygroupid=$1;
 				 shift;
 		 ...
 		 if [[ "$1" == "$CUSTOMERBASED_OPTION" ]]; then
 			 shift;
 			 if (( $# > 0 )) ; then
 				 customerbased=$1;
 				 shift;
 		 ...
 		 if [[ "$1" == "$QUOTATIONFILE_OPTION" ]]; then
 			 shift;
 			 if (( $# > 0 )) ; then
 				 quotationfile=$1;
 				 shift;
	 ...
	 #--------------------------#
 	 ...
 	 #--------------------------#
 	 if [[ "${pharmacygroupid}" == "" ]] || (( 1 != `check_string_parameter "${PHARMACYGROUPID_OPTION}" "${pharmacygroupid}"` )); then
 		 printStartStop "${STARTMSG}"
 		 rc=12;
 		 echo "${PROCNAME}: Parameter value missed or invalid value \${pharmacygroupid}: >${pharmacygroupid}<!" | tee -a ${LOG_RESOURCE}
 		 echo ${USAGE} | tee -a ${LOG_RESOURCE}
 		 printStartStop "${STOPMSG}" "with Exit-Code: <${rc}>"
 		 exit ${rc};
 	 fi
 	 ...
 	 if [[ "${customerbased}" == "" ]] || (( 1 != `check_string_parameter "${CUSTOMERBASED_OPTION}" "${customerbased}"` )); then
 		 printStartStop "${STARTMSG}"
 		 rc=12;
 		 echo "${PROCNAME}: Parameter value missed or invalid value \${customerbased}: >${customerbased}<!" | tee -a ${LOG_RESOURCE}
 		 echo ${USAGE} | tee -a ${LOG_RESOURCE}
 		 printStartStop "${STOPMSG}" "with Exit-Code: <${rc}>"
 		 exit ${rc};
 	 fi
 	 ...
 	 if [[ "${dbserver}" == "" ]] || (( 1 != `check_string_parameter "${DBSERVER_OPTION}" "${dbserver}"` )); then
 		 printStartStop "${STARTMSG}"
 		 rc=12;
 		 echo "${PROCNAME}: Parameter value missed or invalid value \${dbserver}: >${dbserver}<!" | tee -a ${LOG_RESOURCE}
 		 echo ${USAGE} | tee -a ${LOG_RESOURCE}
 		 printStartStop "${STOPMSG}" "with Exit-Code: <${rc}>"
 		 exit ${rc};
 	 fi
 	 ...
 	 if [[ "${dbname}" == "" ]] || (( 1 != `check_string_parameter "${DBNAME_OPTION}" "${dbname}"` )); then
 		 printStartStop "${STARTMSG}"
 		 rc=12;
 		 echo "${PROCNAME}: Parameter value missed or invalid value \${dbname}: >${dbname}<!" | tee -a ${LOG_RESOURCE}
 		 echo ${USAGE} | tee -a ${LOG_RESOURCE}
 		 printStartStop "${STOPMSG}" "with Exit-Code: <${rc}>"
 		 exit ${rc};
 	 fi
 	 ...
 	 if [[ "${quotationfile}" == "" ]] || (( 1 != `check_string_parameter "${QUOTATIONFILE_OPTION}" "${quotationfile}"` )); then
 		 printStartStop "${STARTMSG}"
 		 rc=12;
 		 echo "${PROCNAME}: Parameter value missed or invalid value \${quotationfile}: >${quotationfile}<!" | tee -a ${LOG_RESOURCE}
 		 echo ${USAGE} | tee -a ${LOG_RESOURCE}
 		 printStartStop "${STOPMSG}" "with Exit-Code: <${rc}>"
 		 exit ${rc};
 	 fi
 ...
 function check_string_parameter
 {
 	 ...
 	 if [[ "${_OPTION}" == "${PHARMACYGROUPID_OPTION}" ]]; then
 		 # do the possibly necessary checks with parameter ${_ARG}
 		 # e.g. database server validity: [a-z]*([a-z]|[1-9])@+([a-z]|[1-9]|[_])_@(shm|tcp)
 		 _rc=1;
 	 fi

 	 if [[ "${_OPTION}" == "${CUSTOMERBASED_OPTION}" ]]; then
 		 # do the possibly necessary checks with parameter ${_ARG}
 		 # e.g. database server validity: [a-z]*([a-z]|[1-9])@+([a-z]|[1-9]|[_])_@(shm|tcp)
 		 _rc=1;
 	 fi

 	 if [[ "${_OPTION}" == "${DBSERVER_OPTION}" ]]; then
 		 # do the possibly necessary checks with parameter ${_ARG}
 		 # e.g. database server validity: [a-z]*([a-z]|[1-9])@+([a-z]|[1-9]|[_])_@(shm|tcp)
 		 _rc=1;
 	 fi

 	 if [[ "${_OPTION}" == "${DBNAME_OPTION}" ]]; then
 		 # do the possibly necessary checks with parameter ${_ARG}
 		 # e.g. database server validity: [a-z]*([a-z]|[1-9])@+([a-z]|[1-9]|[_])_@(shm|tcp)
 		 _rc=1;
 	 fi

 	 if [[ "${_OPTION}" == "${QUOTATIONFILE_OPTION}" ]]; then
 		 # do the possibly necessary checks with parameter ${_ARG}
 		 # e.g. database server validity: [a-z]*([a-z]|[1-9])@+([a-z]|[1-9]|[_])_@(shm|tcp)
 		 _rc=1;
 	 fi
 ...
 function assembling_process
 {
	 local _parameters="";
 	 ...
 	 if (( ${#_parameters} > 0 )); then
 		 _parameters="${_parameters} $PHARMACYGROUPID_OPTION $pharmacygroupid $CUSTOMERBASED_OPTION $customerbased $DBSERVER_OPTION $dbserver $DBNAME_OPTION $dbname $QUOTATIONFILE_OPTION $quotationfile"
 	 else
 		 _parameters="$PHARMACYGROUPID_OPTION $pharmacygroupid $CUSTOMERBASED_OPTION $customerbased $DBSERVER_OPTION $dbserver $DBNAME_OPTION $dbname $QUOTATIONFILE_OPTION $quotationfile";
 	 fi
 ...
 assembling_process;
 print_msg "$STARTMSG" "$*"
 export CSCQUOTA_BRANCHNO=34
 export CSCQUOTA_DATE=`date +%Y-%m-%d`
