#! /usr/bin/ksh
 ...
 #Creation-Time:	13:14
 #Purpose:			starts, stops and status the exemplar binary reorgdeliverynotes for presentation only
 ###############################################################################################
 ...
 #--------------------------------------------------------------------#
 readonly BINARY=reorgdeliverynotes.bin
 ...
 readonly DATABASENAME_OPTION="-databasename";
 readonly DELETEDATE_OPTION="-deletedate";
 ...
 {
 	 if [[ "$2" == "" ]]; then set $1 "15.10.08 13:14:45"; fi
 ...
 function print_local_var
 {
	 ...
	 echo "\$DATABASENAME_OPTION:			>$DATABASENAME_OPTION<"
	 echo "\$DELETEDATE_OPTION:			>$DELETEDATE_OPTION<"
 ...
 function eval_parameters
 {
	 ...
	 #--------------------------#
 	 ...
 	 #--------------------------#
 	 if [[ "$databasename" == "" ]] || (( 1 != `check_string_parameter "$databasename"` )); then
 		 printStartStop "$STARTMSG"
 		 rc=12;
 		 echo "$PROCNAME: Parameter value missed or invalid value \$databasename: >$databasename<!" | tee -a $LOG_RESOURCE
 		 echo ${USAGE} | tee -a $LOG_RESOURCE
 		 printStartStop "$STOPMSG" "with Exit-Code: <${rc}>"
 		 exit ${rc};
 	 fi
 	 ...
 	 if [[ "$deletedate" == "" ]] || (( 1 != `check_string_parameter "$deletedate"` )); then
 		 printStartStop "$STARTMSG"
 		 rc=12;
 		 echo "$PROCNAME: Parameter value missed or invalid value \$deletedate: >$deletedate<!" | tee -a $LOG_RESOURCE
 		 echo ${USAGE} | tee -a $LOG_RESOURCE
 		 printStartStop "$STOPMSG" "with Exit-Code: <${rc}>"
 		 exit ${rc};
 	 fi
 ...
 function assembling_process
 {
	 local _parameters="";
 	 ...
 	 if (( ${#_parameters} > 0 )); then
 		 _parameters="${_parameters} $databasename $deletedate"
 	 else
 		 _parameters="$databasename $deletedate";
 	 fi
 ...
 ###############################################################################################
 . $(dirname $0)/reorgdeliverynotes.option
