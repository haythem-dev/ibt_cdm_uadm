#! /usr/bin/ksh
 ...
 #Purpose:           Starts, stops and status the exemplar binary loadflyersubsequent for presentation only.   
 ###############################################################################################
 ...
 #--------------------------------------------------------------------#
 readonly BINARY=loadflyersubsequent.bin
 ...
 readonly IMPORTFILE_OPTION="-inputcsv";
 ...
 function eval_parameters
 {
	 ...
	 #--------------------------#
 	 ...
 	 if [[ "${importfile}" == "" ]]; then
		printStartStop "${STARTMSG}"
		rc=12;
 		echo "${PROCNAME}: Parameter value missed or invalid value \${importfile}: >${importfile}<!" | tee -a ${LOG_RESOURCE}
 		echo ${USAGE} | tee -a ${LOG_RESOURCE}
 		printStartStop "${STOPMSG}" "with Exit-Code: <${rc}>"
 		exit ${rc};
 	 fi
 ...
 function assembling_process
 {
	 local _parameters="";
 	 ...
 	 if [[ "$importfile" != "" ]]; then
 		 _parameters="${_parameters} $IMPORTFILE_OPTION";
 		 _parameters="${_parameters} $importfile";
 	 fi
 ...
 ###############################################################################################
 . $(dirname $0)/loadflyersubsequent.option
 ...
 . $(dirname $0)/../vars/loadflyersubsequent.vars;
