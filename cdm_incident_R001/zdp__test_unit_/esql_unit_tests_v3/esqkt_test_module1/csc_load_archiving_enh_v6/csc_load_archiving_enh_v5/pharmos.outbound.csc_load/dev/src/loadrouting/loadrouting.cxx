...
#define PROJECTNAME	 "loadrouting"
...
int PrepareDeaktivateTours(char *);
int DeaktivateTours(long zeit, long kdnr, char *);
...
int main( int argc, char **argv )
{
	...
	if (lDate == 0) /* wenn keine Datumseingabe dann errechnen Datum */
	{
		...
		printf("lDate = %d",lDate);
		printf("lActDate = %d",lActDate);
	}
	sWeekday = AllgGetWeekDay(lDate,error_msg);
  		
	if ( (rc = PrepareLoadCustomerrouting(error_msg)) < 0) return rc;
	if ( (rc = PrepareDeaktivateTours(error_msg)) < 0) return rc;
...
int PrepareLoadCustomerrouting( char *error_msg)
{
	...
	strcpy(str,"select tourplantag.DATUMKOMMI,tour.ZEITVERSAND from tourplantag,tour ");
	strcat(str," where ");
	...
	//I-24102811 already current day and next day tours, it checks (days 1 to 6) and dose not find any tours
	//already the code was there with nextday = 0 but in the SQL it was checking weekday >= 0 resulting in retriving tours with days 1 to 6
	//so we are now exactly checking with weekday=0
	strcpy(str,"select tourplantagwoche.TOURID from tourplantagwoche,tourplanwoche ");
	strcat(str," where ");
	strcat(str,"tourplantagwoche.WEEKDAY = ?");
	strcat(str," and ");
	...
	EXEC SQL PREPARE sel_tourwoche_0 FROM :str;
	ERROR_MSG2( "prepare sel_tourwoche_0" )
	exec sql declare Cseltourwoche_0 cursor with hold for sel_tourwoche_0;
	ERROR_MSG2( "declare Cseltourwoche_0" )	

	return OK;
}

int PrepareDeaktivateTours( char *error_msg)
{
	char *fun = "PrepareDeaktivateTours";
	int rc;

	strcpy(str,"select tourplantag.TOURINAKTIV from tourplantag ");
	strcat(str," where ");
	strcat(str,"tourplantag.DATUMKOMMI = ?");
	strcat(str," and ");
	strcat(str,"tourplantag.VERTRIEBSZENTRUMNR = ?");
	...
	strcat(str,"tourplantag.TOURINAKTIV = '0'");
	strcat(str," and ");
	strcat(str,"tourplantag.TOURID <= ?");
	strcat(str," for update");

	EXEC SQL PREPARE sel_touraktiv FROM :str;
	ERROR_MSG2( "prepare sel_touraktiv" )
	exec sql declare Cseltouraktiv cursor with hold for sel_touraktiv;
	ERROR_MSG2( "declare sel_touraktiv" )

	strcpy(str,"update tourplantag set ");
	strcat(str,"tourplantag.TOURINAKTIV = '1'");
	strcat(str," where ");
	strcat(str,"current of Cseltouraktiv");

	EXEC SQL PREPARE upd_touraktiv FROM :str;
	ERROR_MSG2( "prepare upd_touraktiv" )
...
int LadeCustomerrouting( char *error_msg)
{
	...
	for(int i=1;;i++)
	{
		...
		if( sqlca.sqlcode == SQLNOTFOUND )
		{
			/* Wenn keine Tour am aktuellen Tag, */
			...
			if( sqlca.sqlcode == SQLNOTFOUND )
			{
				if(sNextday == 0)
				{
					EXEC SQL EXECUTE del_customerrouting USING :sBranchNo,
														:lCustomerNo,
														:sWeekday;
					ERROR_MSG2( "del_customerrouting1:" )
					continue;
				}
				sNextday = 0;
				exec sql open Cseltourwoche_0 using :sNextday,:sDestBranch,:lCustomerNo;
				ERROR_MSG2( "open Cseltourwoche_0" )
				exec sql fetch Cseltourwoche_0 into :cTourID;
				ERROR_MSG2( "fetch Cseltourwoche_0" )
				if( sqlca.sqlcode == SQLNOTFOUND )
				{
					EXEC SQL EXECUTE del_customerrouting USING :sBranchNo,
														:lCustomerNo,
														:sWeekday;
					ERROR_MSG2( "del_customerrouting2:" )
					continue;
				}
			}
			if(atol(cTourID) < lZeitVon)
			{
				EXEC SQL EXECUTE del_customerrouting USING :sBranchNo,
													:lCustomerNo,
													:sWeekday;
				ERROR_MSG2( "del_customerrouting3:" )
				continue;
			}
			lZeitNoTour = lStart = 1;
			lStop = 2359;
			EXEC SQL EXECUTE del_customerrouting USING :sBranchNo,
													:lCustomerNo,
													:sWeekday;
			ERROR_MSG2( "del_customerrouting4:" )
			...
			DeaktivateTours( lZeitNoTour, lCustomerNo, error_msg);
		}
		else
		{
			...
			if( sqlca.sqlcode == SQLNOTFOUND )
			{
				EXEC SQL EXECUTE del_customerrouting USING :sBranchNo,
														:lCustomerNo,
														:sWeekday;
				ERROR_MSG2( "del_customerrouting5:" )
				continue;
			}
			...
			EXEC SQL EXECUTE del_customerrouting USING :sBranchNo,
													:lCustomerNo,
													:sWeekday;
			ERROR_MSG2( "del_customerrouting6:" )
			EXEC SQL EXECUTE ins_customerrouting USING :lCustomerNo,
		                                      :sBranchNo,
											  :lStart,
											  :lStop,
...
int DeaktivateTours( long zeit, long kdnr, char *error_msg)
{
	char *fun = "DeaktivateTours";
	...
	EXEC SQL OPEN Cseltouraktiv using
		:lDate,
		:sDestBranch,
		:lCustomerNo,
		:lZeitrouting;
    ERROR_MSG2( "open Cseltouraktiv" )

	for(;;)
	{
		EXEC SQL fetch Cseltouraktiv into :cInaktiv;
	    ERROR_MSG2( "fetch Cseltouraktiv" )
		if( sqlca.sqlcode == SQLNOTFOUND ) break;
		EXEC SQL EXECUTE upd_touraktiv;
		ERROR_MSG2( "upd_touraktiv:" )
	}
