@echo off
echo Loadflyer transfer process, this is step 3 of 4
set rc=0
...
if exist %drv% (
  ...
  %drv%
  cd \DEPPE1\Abteilung\SRM_WKZRechnungen\Transfer_IDF-gesteuerte_Wannenbeilage\report\
  ...
  echo phosix+>> ftp.txt
  echo ascii>> ftp.txt
  echo cd /software/wss/de/ksc/load/data/flyer/report>> ftp.txt
  echo mget *.csv>> ftp.txt
  echo disconnect>> ftp.txt
  echo quit>> ftp.txt
  ftp -i -s:ftp.txt
  if ERRORLEVEL 1 (
  echo FTP not successfull for csv files
  set rc=2
  ) else (
  echo FTP successull for csv files
  )
  del ftp.txt
  echo Transfer done
  c:
