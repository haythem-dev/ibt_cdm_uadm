#! /usr/bin/ksh
 ...
 #Creation-Time:	11:49
 #Purpose:			starts, stops and status the exemplar binary tpldtour for presentation only
 ###############################################################################################
 ...
 readonly LF_OPTION="-lf";
 readonly LD_OPTION="-ld";
 readonly WD_OPTION="-wd";
 ...
 {
 	 if [[ "$2" == "" ]]; then set $1 "04.09.08 11:49:09"; fi
 ...
 function eval_parameters
 {
	 ...
	 #--------------------------#
 	 ...
 	 #--------------------------#
 	 if [[ "$lf" == "" ]] || (( 1 != `check_numeric_parameter "$lf"` )) || [[ "$lf" == "0" ]]; then
 		 printStartStop "$STARTMSG"
 		 rc=12;
 		 echo "$PROCNAME: Parameter value missed or invalid value \$lf: >$lf<!" | tee -a $LOG_RESOURCE
 		 echo ${USAGE} | tee -a $LOG_RESOURCE
 		 printStartStop "$STOPMSG" "with Exit-Code: <${rc}>"
 		 exit ${rc};
 	 fi
 	 ...
 	 if [[ "$wd" != "" ]]; then
 		 if [[ "$wd" == "" ]] || (( 1 != `check_string_parameter "$wd"` )) || [[ "$wd" == "0" ]]; then
 			 printStartStop "$STARTMSG"
 			 rc=12;
 			 echo "$PROCNAME: Parameter value missed or invalid value \$wd: >$wd<!" | tee -a $LOG_RESOURCE
 			 echo ${USAGE} | tee -a $LOG_RESOURCE
 			 printStartStop "$STOPMSG" "with Exit-Code: <${rc}>"
 			 exit ${rc};
 		 fi
 ...
 function assembling_process
 {
	 local _parameters="";
 	 ...
 	 if [[ "$wd" != "" ]]; then
 		 _parameters="${_parameters} $WD_OPTION ";
 		 _parameters="${_parameters} $wd ";
 	 fi
 	 ...
 	 if (( ${#_parameters} > 0 )); then
 		 _parameters="${_parameters} $LF_OPTION $lf"
 	 else
 		 _parameters="$LF_OPTION $lf";
 	 fi
