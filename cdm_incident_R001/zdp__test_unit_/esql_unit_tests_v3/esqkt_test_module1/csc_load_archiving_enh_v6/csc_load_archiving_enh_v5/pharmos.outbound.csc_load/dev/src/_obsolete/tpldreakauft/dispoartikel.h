...
/******************************************************************************/
/* c:\prj\df\unix\tpldauftr\dispoartikel **************************************/
/******************************************************************************/
...
/* Strucur with real chartype of DISPOARTIKEL *********************************/
...
/* Sizetype  Strucur (buf_desc) of DISPOARTIKEL *******************************/
...
class CDISPOARTIKEL : public ppDadeVirtual
{
	...
	public:
		...
		bool 		DelList(const unsigned int i) { if (GetSize() <= i) return false; vector<dispoartikelS>::iterator itPtr = pl->begin() + i; if ( itPtr ) pl->erase(itPtr); if ( --AnzPos < 0 ) AnzPos = 0; return true; }
		...
		const long		GetPosnr() const	{ return s.POSNR; }
		...
		const long		GetEtartklasse1() const	{ return s.ETARTKLASSE1; }
		const long		GetEtartklasse2() const	{ return s.ETARTKLASSE2; }
		const long		GetEtartschalter1() const	{ return s.ETARTSCHALTER1; }
		const long		GetEtartschalter2() const	{ return s.ETARTSCHALTER2; }
		const long		GetEtartschalter3() const	{ return s.ETARTSCHALTER3; }
		const long		GetEtartschalter4() const	{ return s.ETARTSCHALTER4; }
		const long		GetEtartschalter5() const	{ return s.ETARTSCHALTER5; }
		const double		GetPreisekapo() const	{ return s.PREISEKAPO; }
		const double		GetPreisekgrosso() const	{ return s.PREISEKGROSSO; }
		const double		GetRabattfestdm() const	{ return s.RABATTFESTDM; }
		const double		GetRabattfest() const	{ return s.RABATTFEST; }
		...
		const double		GetGebuehrbearb() const	{ return s.GEBUEHRBEARB; }
		const long		GetUrsprungauftragnr() const	{ return s.URSPRUNGAUFTRAGNR; }
		const long		GetUrsprungposnr() const	{ return s.URSPRUNGPOSNR; }
...
/* CreateSqlStatement of DISPOARTIKEL *****************************************/
 ...
 #define DISPOARTIKEL_CREATE(x) exec sql create table x (\
         ...
         URSPRUNGPOSNR integer default 0 not null) in phosix extent size 102 lock mode row;
