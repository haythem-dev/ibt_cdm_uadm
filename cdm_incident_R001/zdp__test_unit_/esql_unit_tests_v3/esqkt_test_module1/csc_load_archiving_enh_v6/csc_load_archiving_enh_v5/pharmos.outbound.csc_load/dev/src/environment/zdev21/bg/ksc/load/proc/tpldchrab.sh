#! /usr/bin/ksh
 ...
 #Creation-Time:	11:28
 #Purpose:			starts, stops and status the exemplar binary tpldchrab for presentation only
 ###############################################################################################
 ...
 {
 	 if [[ "$2" == "" ]]; then set $1 "19.09.08 11:28:04"; fi
 ...
 function eval_parameters
 {
	 ...
	 #--------------------------#
 	 ...
 	 if [[ "$file" == "" ]] || (( 1 != `check_string_parameter "$file"` )); then
 		 printStartStop "$STARTMSG"
 		 rc=12;
 		 echo "$PROCNAME: Parameter value missed or invalid value \$file: >$file<!" | tee -a $LOG_RESOURCE
 		 echo ${USAGE} | tee -a $LOG_RESOURCE
 		 printStartStop "$STOPMSG" "with Exit-Code: <${rc}>"
 		 exit ${rc};
 	 fi
...
# 	 if (( ${#_parameters} > 0 )); then
# 		 _parameters="${_parameters} $VZ_OPTION $vz $FILE_OPTION $file"
# 	 else
# 		 _parameters="$VZ_OPTION $vz $FILE_OPTION $file";
# 	 fi
