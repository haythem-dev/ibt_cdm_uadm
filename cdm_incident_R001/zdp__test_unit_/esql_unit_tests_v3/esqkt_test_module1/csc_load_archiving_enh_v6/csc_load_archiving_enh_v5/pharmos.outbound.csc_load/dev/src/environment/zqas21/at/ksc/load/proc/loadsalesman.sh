#! /usr/bin/ksh
 ...
 #Purpose:           Starts, stops and status the exemplar binary loadsalesman for presentation only.          
 ###############################################################################################
 ...
 #--------------------------------------------------------------------#
 readonly BINARY=loadsalesman.bin
 ...
 function eval_parameters
 {
	 ...
	 #--------------------------#
 	 ...
 	 #--------------------------#
 	 if [[ "${branchno}" == "" ]] || (( 1 != `check_string_parameter "${BRANCHNO_OPTION}" "${branchno}"` )); then
 		 printStartStop "${STARTMSG}"
 		 rc=12;
 ...
 . $(dirname $0)/../vars/loadsalesman.vars;
