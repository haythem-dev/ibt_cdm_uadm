...
/******************************************************************************/
/* d:\prj\DF\UNIX\loadcstdiscacc\CstDiscAcc ***********************************/
/******************************************************************************/
...
/* Length/Count-Define of CSTDISCACC ******************************************/
  ...
  #define LS_CSTDISCACC_UPD_FLAG 1 + 1
  ...
  #define CSTDISCACCUPD_FLAG 4
  #define CSTDISCACCKDAUFTRAGNR 5
  #define CSTDISCACCDATE 6
  #define CSTDISCACCINVOICENUMBER 7
  #define CSTDISCACCINVOICEAMOUNT 8
  #define CSTDISCACCVOUCHERTYP 9
  #define CSTDISCACCPREIS_TYP 10
  #define CSTDISCACCPRIORITY 11
  #define CSTDISCACCKDAUFTRAGNRSPLIT 12
  #define CSTDISCACCINVOICEPAID 13
  #define CSTDISCACCPAYMENTARGET 14
  #define CSTDISCACCLFDNR 15
...
/* SqlMacros-Define of CSTDISCACC *********************************************/
 ...
 #define CSTDISCACC_UPDATE(x) \
         ...
         CSTDISCACC.UPD_FLAG=:x->UPD_FLAG,\
...
class CCSTDISCACC : public ppDadeVirtual
{
	...
	private:
		...
		int  CursorPos;
		int  AnzPos;
		vector<cstdiscaccS> *pl; // pointer to class list
	...
	public:
		...
		bool 		SetList(const unsigned int i) { if (GetSize() <= i) return false; vector<cstdiscaccS>::iterator itPtr = pl->begin() + i; *itPtr = s; return true; }
		bool 		InsList() { if ( !pl ) pl = new vector<cstdiscaccS>; Strip(s); cstdiscaccS c = s; pl->insert(pl->end(),c); AnzPos++; return true; }
		bool 		DelList(const unsigned int i) { if (GetSize() <= i) return false; vector<cstdiscaccS>::iterator itPtr = pl->begin() + i; if ( itPtr < pl->end() ) pl->erase(itPtr); if ( --AnzPos < 0 ) AnzPos = 0; return true; }
		vector<cstdiscaccS>::iterator begin(){ if (pl) return pl->begin(); else return NULL; };
		vector<cstdiscaccS>::iterator end(){ if (pl) return pl->end(); else return NULL; };
		...
		const cstdiscaccS& GetStruct() const { return s; }
		...
		int 		CheckExist(int FetchRel = 1,int pos=0)	{ int rc = CursorServerCall(1522,FetchRel,pos); return rc;}
		int 		SelListCst(int FetchRel = 1,int pos=0)	{ int rc = CursorServerCall(1520,FetchRel,pos); return rc;}
		int 		SelListGrp(int FetchRel = 1,int pos=0)	{ int rc = CursorServerCall(1521,FetchRel,pos); return rc;}
	...
	public:
		...
		int CloseCursor(int pos = 0)  //close db cursor
		{
			...
			rc = ::ServerNr(pos, FncNumber, (void *)&s,CSTDISCACC_BES,CSTDISCACC_ANZ, error_msg);
			#else
		...
		{
			if( fkt_nr == 1522 ) 	return CursorServerCall(1522,s.FETCH_REL,pos);
			if( fkt_nr == 1520 ) 	return CursorServerCall(1520,s.FETCH_REL,pos);
			if( fkt_nr == 1521 ) 	return CursorServerCall(1521,s.FETCH_REL,pos);
...
/* CreateSqlStatement of CSTDISCACC *******************************************/
 ...
 #define CSTDISCACC_CREATE(x) exec sql create table x (\
         ...
         LFDNR integer default 0 not null) in ksc extent size 40 next size 8 lock mode row;
