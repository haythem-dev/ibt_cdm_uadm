...
/******************************************************************************/
/* d:\prj\DF\UNIX\tpldtour\tourplanwoche **************************************/
/******************************************************************************/
...
/* original Strucur of TOURPLANWOCHE ******************************************/
...
class CTOURPLANWOCHE : public ppDadeVirtual
{
	...
	private:
		...
		int  CursorPos;
		int  AnzPos;
		vector<tourplanwocheS> *pl; // pointer to class list
	...
	public:
		...
		bool 		SetList(const unsigned int i) { if (GetSize() <= i) return false; vector<tourplanwocheS>::iterator itPtr = pl->begin() + i; *itPtr = s; return true; }
		bool 		InsList() { if ( !pl ) pl = new vector<tourplanwocheS>; Strip(s); tourplanwocheS c = s; pl->insert(pl->end(),c); AnzPos++; return true; }
		bool 		DelList(const unsigned int i) { if (GetSize() <= i) return false; vector<tourplanwocheS>::iterator itPtr = pl->begin() + i; if ( itPtr < pl->end() ) pl->erase(itPtr); if ( --AnzPos < 0 ) AnzPos = 0; return true; }
		vector<tourplanwocheS>::iterator begin(){ if (pl) return pl->begin(); else return NULL; };
		vector<tourplanwocheS>::iterator end(){ if (pl) return pl->end(); else return NULL; };
		...
		const tourplanwocheS& GetStruct() const { return s; }
	...
	public:
		...
		virtual ~CTOURPLANWOCHE()  //destructor
		{
			if(pl) {
				delete pl;
				pl = NULL;
		...
		{
			#ifdef WIN32
			rc = ::ServerNr(pos, fkt_nr, (void *)&s, TOURPLANWOCHE_BES, TOURPLANWOCHE_ANZ, error_msg);
			#else
			rc = ::Server(fkt_nr, (void *)&s, TOURPLANWOCHE_BES, TOURPLANWOCHE_ANZ, error_msg);
			#endif
	...
	private:
		...
		int UniqueServerCall(int FncNr,int pos=0)
		{
			...
			rc = ::ServerNr(pos, FncNr, (void *)&s, TOURPLANWOCHE_BES, TOURPLANWOCHE_ANZ, error_msg);
			#else
...
/* CreateSqlStatement of TOURPLANWOCHE ****************************************/
 ...
 #define TOURPLANWOCHE_CREATE(x) exec sql create table x (\
         ...
         WEEKDAY smallint default 0 not null) in ksc extent size 284 next size 32 lock mode row;
