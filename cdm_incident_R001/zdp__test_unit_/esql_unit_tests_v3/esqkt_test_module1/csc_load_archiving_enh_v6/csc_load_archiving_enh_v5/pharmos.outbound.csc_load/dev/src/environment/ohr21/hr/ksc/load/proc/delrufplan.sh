#! /usr/bin/ksh
 ...
 #Creation-Time:	15:14
 #Purpose:			starts, stops and status the exemplar binary delrufplan for presentation only
 ###############################################################################################
 ...
 #--------------------------------------------------------------------#
 readonly BINARY=delrufplan.bin
 ...
 readonly BACKUP_OPTION="-backup";    #Ronnie
 readonly KZ_OPTION="-kz";
 ...
 {
 	 if [[ "$2" == "" ]]; then set $1 "04.09.08 15:14:38"; fi
 ...
 function print_local_var
 {
 	 ...
 	 echo "\$BACKUP_OPTION:			>$BACKUP_OPTION<"    #Ronnie
	 echo "\$KZ_OPTION:			>$KZ_OPTION<"
 ...
 function eval_parameters
 {
 	 ...
 	 do
 		 ...
 		 if [[ "$1" == "$BACKUP_OPTION" ]] ; then backup="e"; shift; continue; fi    #Ronnie
	 ...
	 #--------------------------#
 	 ...
 	 #--------------------------#
 	 if [[ "$kz" == "" ]] || (( 1 != `check_string_parameter "$kz"` )); then
 		 printStartStop "$STARTMSG"
 		 rc=12;
 		 echo "$PROCNAME: Parameter value missed or invalid value \$kz: >$kz<!" | tee -a $LOG_RESOURCE
 		 echo ${USAGE} | tee -a $LOG_RESOURCE
 		 printStartStop "$STOPMSG" "with Exit-Code: <${rc}>"
 		 exit ${rc};
 	 fi
 ...
 function assembling_process
 {
	 local _parameters="";
 	 ...
 	 if (( ${#_parameters} > 0 )); then
 		 _parameters="${_parameters} $KZ_OPTION $kz"
 	 else
 		 _parameters="$KZ_OPTION $kz";
 	 fi
 ...
 . $(dirname $0)/../vars/delrufplan.vars;
