#! /usr/bin/ksh
 ...
 #Creation-Time:	11:51
 #Purpose:			starts, stops and status the exemplar binary tpldhipath for presentation only
 ###############################################################################################
 ...
 #LIBRARY PATH 
 readonly LIBRARY_PATH=${LIB_PATH}/servclnt/01.16.00.00/sharedlib/release
 export LD_LIBRARY_PATH=${LIBRARY_PATH}:$LD_LIBRARY_PATH
 readonly BINARY=tpldhipath.bin
 process="${BIN_PATH}/${BINARY}"
 process2="${BIN_PATH}/${BINARY}"
 ...
 {
 	 if [[ "$2" == "" ]]; then set $1 "23.10.08 11:51:33"; fi
 ...
 function eval_parameters
 {
	 ...
	 #--------------------------#
 	 ...
 	 #--------------------------#
 	 if [[ "$dc" == "" ]] || (( 1 != `check_numeric_parameter "$dc"` )) || [[ "$dc" == "0" ]]; then
 		 printStartStop "$STARTMSG"
 		 rc=12;
 		 echo "$PROCNAME: Parameter value missed or invalid value \$dc: >$dc<!" | tee -a $LOG_RESOURCE
 		 echo ${USAGE} | tee -a $LOG_RESOURCE
 		 printStartStop "$STOPMSG" "with Exit-Code: <${rc}>"
 		 exit ${rc};
 	 fi
 ...
 function assembling_process
 {
	 ...
	 local _parameters2="";
 	 ...
 	 if (( ${#_parameters} > 0 )); then
 		 _parameters="${_parameters} $DC_OPTION $dc"
 	 else
 		 _parameters="$DC_OPTION $dc";
 	 fi
 	 ...
 	 if (( ${#ADDITIONAL_PARA2} > 0 )); then
 		 if (( ${#_parameters2} > 0 )); then
 			 _parameters2="${_parameters2} ${ADDITIONAL_PARA2}";
 		 else
 			 _parameters2="${ADDITIONAL_PARA2}";
 		 fi
 	 fi
 	 ...
 	 if (( ${#_parameters2} > 0 )); then
 		 process2="$process2 ${_parameters2}"
 	 fi
 ...
 . $(dirname $0)/../vars/tpldhipath.vars;
