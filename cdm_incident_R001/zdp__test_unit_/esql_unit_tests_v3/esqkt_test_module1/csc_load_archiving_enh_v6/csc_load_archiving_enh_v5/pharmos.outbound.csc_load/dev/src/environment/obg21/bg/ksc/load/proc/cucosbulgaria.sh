#! /usr/bin/ksh
 ...
 #Purpose:           Starts, stops and status the exemplar binary cucosbulgaria for presentation only.         
 ###############################################################################################
 ...
 #--------------------------------------------------------------------#
 readonly BINARY=cucosbulgaria._sh
 ...
 function eval_parameters
 {
	 ...
	 #--------------------------#
 	 ...
 	 if [[ "${tidy}" == "" ]] || (( 1 != `check_string_parameter "${TIDY_OPTION}" "${tidy}"` )); then
 		 printStartStop "${STARTMSG}"
 		 rc=12;
 		 echo "${PROCNAME}: Parameter value missed or invalid value \${tidy}: >${tidy}<!" | tee -a ${LOG_RESOURCE}
 		 echo ${USAGE} | tee -a ${LOG_RESOURCE}
 		 printStartStop "${STOPMSG}" "with Exit-Code: <${rc}>"
 		 exit ${rc};
 	 fi
 ...
 function assembling_process
 {
	 local _parameters="";
 	 ...
 	 if (( ${#_parameters} > 0 )); then
 		 _parameters="${_parameters} $KSCDB_OPTION $kscdb $BRANCHNO_OPTION $branchno $CUCOSTABLE_OPTION $cucosTable $TIDY_OPTION $tidy"
 	 else
 		 _parameters="$KSCDB_OPTION $kscdb $BRANCHNO_OPTION $branchno $CUCOSTABLE_OPTION $cucosTable $TIDY_OPTION $tidy";
 	 fi
 ...
 ###############################################################################################
 . $(dirname $0)/cucosbulgaria.option
