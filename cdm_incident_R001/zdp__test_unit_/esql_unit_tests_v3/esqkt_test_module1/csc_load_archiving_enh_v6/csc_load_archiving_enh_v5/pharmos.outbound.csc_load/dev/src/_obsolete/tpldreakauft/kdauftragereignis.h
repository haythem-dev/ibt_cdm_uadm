...
/******************************************************************************/
/* d:\prj\DF\UNIX\tpldauftr\kdauftragereignis *********************************/
/******************************************************************************/
...
/* original Strucur of KDAUFTRAGEREIGNIS **************************************/
...
class CKDAUFTRAGEREIGNIS : public ppDadeVirtual
{
	...
	private:
		...
		int  CursorPos;
		int  AnzPos;
		vector<kdauftragereignisS> *pl; // pointer to class list
	...
	public:
		...
		bool 		SetList(const unsigned int i) { if (GetSize() <= i) return false; vector<kdauftragereignisS>::iterator itPtr = pl->begin() + i; *itPtr = s; return true; }
		bool 		InsList() { if ( !pl ) pl = new vector<kdauftragereignisS>; Strip(s); kdauftragereignisS c = s; pl->insert(pl->end(),c); AnzPos++; return true; }
		bool 		DelList(const unsigned int i) { if (GetSize() <= i) return false; vector<kdauftragereignisS>::iterator itPtr = pl->begin() + i; if ( itPtr < pl->end() ) pl->erase(itPtr); if ( --AnzPos < 0 ) AnzPos = 0; return true; }
		vector<kdauftragereignisS>::iterator begin(){ if (pl) return pl->begin(); else return NULL; };
		vector<kdauftragereignisS>::iterator end(){ if (pl) return pl->end(); else return NULL; };
		...
		const kdauftragereignisS& GetStruct() const { return s; }
	...
	public:
		...
		CKDAUFTRAGEREIGNIS()  //constructor
		{
		...
		virtual ~CKDAUFTRAGEREIGNIS()  //destructor
		{
			if(pl) {
				delete pl;
				pl = NULL;
		...
		{
			#ifdef WIN32
			rc = ::ServerNr(pos, fkt_nr, (void *)&s, KDAUFTRAGEREIGNIS_BES, KDAUFTRAGEREIGNIS_ANZ, error_msg);
			#else
			rc = ::Server(fkt_nr, (void *)&s, KDAUFTRAGEREIGNIS_BES, KDAUFTRAGEREIGNIS_ANZ, error_msg);
			#endif
	...
	private:
		...
		int UniqueServerCall(int FncNr,int pos=0)
		{
			...
			rc = ::ServerNr(pos, FncNr, (void *)&s, KDAUFTRAGEREIGNIS_BES, KDAUFTRAGEREIGNIS_ANZ, error_msg);
			#else
...
/* CreateSqlStatement of KDAUFTRAGEREIGNIS ************************************/
 ...
 #define KDAUFTRAGEREIGNIS_CREATE(x) exec sql create table x (\
         ...
         FILIALNR smallint default 0 not null) in KscOrder extent size 5100 next size 512 lock mode row;
