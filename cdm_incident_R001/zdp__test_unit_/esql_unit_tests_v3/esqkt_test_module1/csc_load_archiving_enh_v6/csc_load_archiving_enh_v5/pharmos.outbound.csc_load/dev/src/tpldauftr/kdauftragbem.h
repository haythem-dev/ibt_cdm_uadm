#ifndef __PP_KDAUFTRAGBEM_H_
#define __PP_KDAUFTRAGBEM_H_

/******************************************************************************/
/* d:\prj\DF\UNIX\tpldauftr\kdauftragbem **************************************/
/******************************************************************************/
...
/* SqlMacros-Define of KDAUFTRAGBEM *******************************************/
 ...
 #define KDAUFTRAGBEM_ZEIGERSEL(x) \
         :x->KDAUFTRAGNR,\
         :x->BEMERKUNGSART,\
         :x->BEMERKUNGSNR,\
         :x->BEMERKUNGEN
...
/* Structur with real chartype of KDAUFTRAGBEM ********************************/
...
/* Sizetype  Structur (buf_desc) of KDAUFTRAGBEM ******************************/
...
class CKDAUFTRAGBEM : public ppDadeVirtual
{
	...
	public:
		...
		bool 		DelList(const unsigned int i) { if (GetSize() <= i) return false; vector<kdauftragbemS>::iterator itPtr = pl->begin() + i; if ( itPtr < pl->end() ) pl->erase(itPtr); if ( --AnzPos < 0 ) AnzPos = 0; return true; }
		...
		short		GetBemerkungsart() const	{ return s.BEMERKUNGSART; }
		short		GetBemerkungsnr() const	{ return s.BEMERKUNGSNR; }
	...
	public:
		...
		virtual ~CKDAUFTRAGBEM()  //destructor
		{
			if(pl) {
				delete pl;
				pl = NULL;
		...
		{
			#ifdef WIN32
			rc = ::ServerNr(pos, fkt_nr, (void *)&s, KDAUFTRAGBEM_BES, KDAUFTRAGBEM_ANZ, error_msg);
			#else
			rc = ::Server(fkt_nr, (void *)&s, KDAUFTRAGBEM_BES, KDAUFTRAGBEM_ANZ, error_msg);
			#endif
...
/* CreateSqlStatement of KDAUFTRAGBEM *****************************************/
 ...
 #define KDAUFTRAGBEM_CREATE(x) exec sql create table x (\
         ...
         BEMERKUNGEN varchar(240,0) default " " not null) in KscOrder extent size 32 next size 8 lock mode row;
