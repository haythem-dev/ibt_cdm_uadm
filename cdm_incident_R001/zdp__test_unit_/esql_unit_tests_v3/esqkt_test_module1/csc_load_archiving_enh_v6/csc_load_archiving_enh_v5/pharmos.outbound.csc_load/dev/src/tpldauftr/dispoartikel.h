...
/******************************************************************************/
/* c:\Marathon\orderentry\csc_load\trunk\dev\src\tpldauftr\dispoartikel *******/
...
/* original struct of DISPOARTIKEL ********************************************/
...
private:
    ...
    int  CursorPos;
    int  AnzPos;
    vector<dispoartikelS> lst; // class list
...
public:
    ...
    bool         SetList(const unsigned int i) {
                     if (GetSize() <= i)
                         return false;
                     vector< dispoartikelS >::iterator it = lst.begin() + i;
                     *it = s;
                     return true;
                 }
    bool         InsList() {
...
public:
    ...
    #ifdef WIN32
        rc = ::ServerNr(pos, fkt_nr, (void *)&s, DISPOARTIKEL_BES, (int)DISPOARTIKEL_ANZ, error_msg);
    #else
        rc = ::Server(fkt_nr, (void *)&s, DISPOARTIKEL_BES, (int)DISPOARTIKEL_ANZ, error_msg);
    #endif
        if (rc < 0)
...
private:
    ...
    int UniqueServerCall(int FncNr, int pos = 0) {
       ...
       rc = ::ServerNr(pos, FncNr, (void *)&s, DISPOARTIKEL_BES, (int)DISPOARTIKEL_ANZ, error_msg);
    #else
       pos = pos;
