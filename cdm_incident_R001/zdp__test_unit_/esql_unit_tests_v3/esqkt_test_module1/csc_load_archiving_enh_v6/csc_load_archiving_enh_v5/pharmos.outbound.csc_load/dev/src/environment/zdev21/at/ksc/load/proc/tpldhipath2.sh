#! /usr/bin/ksh
 ...
 #Creation-Time:     15:38                                                                                     
 #Purpose:           Starts, stops and status the exemplar binary tpldhipath2 for presentation only.           
 ###############################################################################################
 ...
 readonly BINARY=tpldhipath2.bin
 ...
 . $(dirname $0)/../vars/tpldhipath2.vars;
 ...
 if [[ "$start" == "YES" ]]; then
	 echo "$0: $process"
 	 msg=`start_process "${process}" "-NOHUP" "-BG" "-SLEEP" "0"`
 	 #start_process "${process}" "-NOHUP" "-BG" 
 	 rc=$?;
	 readonly TECHNICAL_PROBLEMS=2;
	 if (( ${TECHNICAL_PROBLEMS} == ${rc} )); then
		rc=0;
	 fi
