#! /usr/bin/ksh
 ...
 #Creation-Time:	14:53
 #Purpose:			starts, stops and status the exemplar binary tpldzentral for presentation only
 ###############################################################################################
 ...
 #--------------------------------------------------------------------#
 readonly BINARY=tpldzentral.bin
 ...
 {
 	 if [[ "$2" == "" ]]; then set $1 "05.09.08 14:53:21"; fi
 ...
 function assembling_process
 {
	 local _parameters="";
 	 ...
 	 if (( ${all_flag} == 1 )); then
 		 _parameters="${_parameters} $ALL_OPTION ";
 	 fi
 ...
 . $(dirname $0)/../vars/tpldzentral.vars;
