...
#===========================================================#
 ...
 function process_loadquotabyfile
 {
 	 ...
 	 typeset -r _db="${1}";
 	 typeset -r _dbsrv="${2}";
	 typeset -r _filepath="${3}";
	 typeset -r _filename="${4}";
	 typeset -r _insert_columns="${5}";
	 typeset -r _delimiter="${6}";
	 ...
	 typeset -r _full_loadtablename="${_full_db}:articlequota_load";
	 typeset -r _sql_load_statement="create temp table ${_full_loadtablename} ( \
														branchno smallint, \
														customerno int, \
														articleno int, \
														quota int, \
														...
														datefrom int, \
														dateto int); 
									 load from '${_full_filename}' delimiter '${_delimiter}' insert into ${_full_loadtablename} (${_insert_columns});\
									 merge into ${_full_tablename} aq \
									 using ${_full_loadtablename} lq \
									 on aq.customerno = lq.customerno and aq.articleno = lq.articleno \
									 when matched then \
									 update set aq.quota = lq.quota , aq.validity = lq.validity , aq.datefrom = lq.datefrom , aq.dateto = lq.dateto \
									 when not matched then \
									 insert (aq.branchno,aq.customerno,aq.articleno,aq.quota,aq.validity,aq.datefrom,aq.dateto) 
										values (lq.branchno,lq.customerno,lq.articleno,lq.quota,lq.validity,lq.datefrom,lq.dateto);";	
	 ...
	 do_sql "${_sql_load_statement}" "${_full_db}" ;
	 rc=${?};
 ...
 if [[ "$start" == "YES" ]]; then
 	 ...
 	 echo "#-----------------------------------------------------------------#" | tee -a ${LOG_RESOURCE}
 	 process_loadquotabyfile "${db}" "${dbsrv}" "${DATA_PATH}" "${filename}" "${INSERT_COLUMNS}" "${FILE_FIELD_DELIMITER}"
 	 rc=${?};
 	 if (( ${ERROR} == ${rc} )); then
 		 exit ${rc};
 	 fi
