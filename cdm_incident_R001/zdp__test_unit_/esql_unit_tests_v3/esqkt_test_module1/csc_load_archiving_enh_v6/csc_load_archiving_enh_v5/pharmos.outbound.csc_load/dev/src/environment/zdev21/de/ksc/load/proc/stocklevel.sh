#! /usr/bin/ksh
 ...
 #Creation-Time:     15:15                                                                                     
 #Purpose:           Starts, stops and status the exemplar binary stocklevel for presentation only.            
 ###############################################################################################
 ...
 #--------------------------------------------------------------------#
 readonly BINARY=stocklevel._sh
 process="${PROC_PATH}/${BINARY}"
 ...
 readonly CF_OPTION="-cf";
 readonly AF_OPTION="-af";
 ...
 function eval_parameters
 {
	 ...
	 #--------------------------#
 	 ...
 	 #--------------------------#
 	 if [[ "${cf}" == "" ]] || (( 1 != `check_string_parameter "${CF_OPTION}" "${cf}"` )); then
 		 printStartStop "${STARTMSG}"
 		 rc=12;
 		 echo "${PROCNAME}: Parameter value missed or invalid value \${cf}: >${cf}<!" | tee -a ${LOG_RESOURCE}
 		 echo ${USAGE} | tee -a ${LOG_RESOURCE}
 		 printStartStop "${STOPMSG}" "with Exit-Code: <${rc}>"
 		 exit ${rc};
 	 fi
 	 ...
 	 if [[ "${af}" == "" ]] || (( 1 != `check_string_parameter "${AF_OPTION}" "${af}"` )); then
 		 printStartStop "${STARTMSG}"
 		 rc=12;
 		 echo "${PROCNAME}: Parameter value missed or invalid value \${af}: >${af}<!" | tee -a ${LOG_RESOURCE}
 		 echo ${USAGE} | tee -a ${LOG_RESOURCE}
 		 printStartStop "${STOPMSG}" "with Exit-Code: <${rc}>"
 		 exit ${rc};
 	 fi
 	 ...
 	 if [[ "${m}" == "" ]] || (( 1 != `check_string_parameter "${M_OPTION}" "${m}"` )); then
 		 printStartStop "${STARTMSG}"
 		 rc=12;
 		 echo "${PROCNAME}: Parameter value missed or invalid value \${m}: >${m}<!" | tee -a ${LOG_RESOURCE}
 		 echo ${USAGE} | tee -a ${LOG_RESOURCE}
 		 printStartStop "${STOPMSG}" "with Exit-Code: <${rc}>"
 		 exit ${rc};
 	 fi
 ...
 function assembling_process
 {
	 local _parameters="";
 	 ...
 	 if (( ${#_parameters} > 0 )); then
 		 _parameters="${_parameters} $CF_OPTION $cf $AF_OPTION $af $KSCDB_OPTION $kscdb $M_OPTION $m"
 	 else
 		 _parameters="$CF_OPTION $cf $AF_OPTION $af $KSCDB_OPTION $kscdb $M_OPTION $m";
 	 fi
 ...
 if [[ "$start" == "YES" ]]; then
 	 ...
 	 #+++ echo "$msg" | tee -a $LOG_RESOURCE
          ...
          _parameters="$CF_OPTION $cf $AF_OPTION $af $KSCDB_OPTION $kscdb $M_OPTION $m";
         #+++ . ./stocklevel._sh _parameters
         #. ~/Prozedurgenerator/de/ksc/load/proc/stocklevel._sh _parameters
 	 process="${process} ${_parameters}"
 	 ${process};
 	 rc=$?;
 fi
