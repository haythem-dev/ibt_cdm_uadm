#! /usr/bin/ksh
 ...
 #Purpose:           Starts, stops and status the exemplar binary grpr2disc for presentation only.             
 ###############################################################################################
 ...
 readonly CUCOSTABLE_OPTION="-cucosTable";
 readonly BRANCH_OPTION="-branch";
 readonly MAX_OPTION="-max";
 ###############################################################################################
 ...
 function print_local_var
 {
	 ...
	 printf "%-30s%-90s\n" "\$DB_OPTION:" ">$DB_OPTION<"
	 printf "%-30s%-90s\n" "\$CUCOSTABLE_OPTION:" ">$CUCOSTABLE_OPTION<"
 ...
 function eval_parameters
 {
	 ...
	 #--------------------------#
 	 ...
 	 if [[ "${cucosTable}" == "" ]] || (( 1 != `check_string_parameter "${CUCOSTABLE_OPTION}" "${cucosTable}"` )); then
 		 printStartStop "${STARTMSG}"
 		 rc=12;
 		 echo "${PROCNAME}: Parameter value missed or invalid value \${cucosTable}: >${cucosTable}<!" | tee -a ${LOG_RESOURCE}
 		 echo ${USAGE} | tee -a ${LOG_RESOURCE}
 		 printStartStop "${STOPMSG}" "with Exit-Code: <${rc}>"
 		 exit ${rc};
 	 fi
 ...
 function check_string_parameter
 {
 	 ...
 	 if [[ "${_OPTION}" == "${CUCOSTABLE_OPTION}" ]]; then
 		 # do the possibly necessary checks with parameter ${_ARG}
 		 # e.g. database server validity: [a-z]*([a-z]|[1-9])@+([a-z]|[1-9]|[_])_@(shm|tcp)
 		 _rc=1;
 	 fi
 ...
 function assembling_process
 {
	 local _parameters="";
 	 if (( $max > 0 )); then
 		 _parameters="${_parameters} $MAX_OPTION ";
 		 _parameters="${_parameters} $max ";
 	 fi
 	 ...
 	 if (( ${#_parameters} > 0 )); then
 		 _parameters="${_parameters} $DBSERVER_OPTION $dbserver $DB_OPTION $db $CUCOSTABLE_OPTION $cucosTable $BRANCH_OPTION $branch"
 	 else
 		 _parameters="$DBSERVER_OPTION $dbserver $DB_OPTION $db $CUCOSTABLE_OPTION $cucosTable $BRANCH_OPTION $branch";
 	 fi
