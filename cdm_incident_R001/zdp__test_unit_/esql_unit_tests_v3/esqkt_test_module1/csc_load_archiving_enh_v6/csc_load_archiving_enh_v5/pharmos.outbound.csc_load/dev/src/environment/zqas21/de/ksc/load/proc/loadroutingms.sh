#! /usr/bin/ksh
 ...
 #Creation-Time:	12:54
 #Purpose:			starts, stops and status the exemplar binary loadroutingms for presentation only
 ###############################################################################################
 ...
 #--------------------------------------------------------------------#
 readonly BINARY=loadroutingms.bin
 ...
 readonly DEST_OPTION="-dest";
 readonly VON_OPTION="-von";
 readonly BIS_OPTION="-bis";
 readonly LD_OPTION="-ld";
 ...
 {
 	 if [[ "$2" == "" ]]; then set $1 "14.10.08 12:54:00"; fi
 ...
 function print_local_var
 {
	 ...
	 echo "\$DEST_OPTION:			>$DEST_OPTION<"
	 echo "\$VON_OPTION:			>$VON_OPTION<"
	 echo "\$BIS_OPTION:			>$BIS_OPTION<"
 ...
 function eval_parameters
 {
	 ...
	 #--------------------------#
 	 ...
 	 if [[ "$dest" == "" ]] || (( 1 != `check_numeric_parameter "$dest"` )) || [[ "$dest" == "0" ]]; then
 		 printStartStop "$STARTMSG"
 		 rc=12;
 		 echo "$PROCNAME: Parameter value missed or invalid value \$dest: >$dest<!" | tee -a $LOG_RESOURCE
 		 echo ${USAGE} | tee -a $LOG_RESOURCE
 		 printStartStop "$STOPMSG" "with Exit-Code: <${rc}>"
 		 exit ${rc};
 	 fi
 	 ...
 	 if [[ "$von" == "" ]] || (( 1 != `check_numeric_parameter "$von"` )) || [[ "$von" == "0" ]]; then
 		 printStartStop "$STARTMSG"
 		 rc=12;
 		 echo "$PROCNAME: Parameter value missed or invalid value \$von: >$von<!" | tee -a $LOG_RESOURCE
 		 echo ${USAGE} | tee -a $LOG_RESOURCE
 		 printStartStop "$STOPMSG" "with Exit-Code: <${rc}>"
 		 exit ${rc};
 	 fi
 	 ...
 	 if [[ "$bis" == "" ]] || (( 1 != `check_numeric_parameter "$bis"` )) || [[ "$bis" == "0" ]]; then
 		 printStartStop "$STARTMSG"
 		 rc=12;
 		 echo "$PROCNAME: Parameter value missed or invalid value \$bis: >$bis<!" | tee -a $LOG_RESOURCE
 		 echo ${USAGE} | tee -a $LOG_RESOURCE
 		 printStartStop "$STOPMSG" "with Exit-Code: <${rc}>"
 		 exit ${rc};
 	 fi
 	 ...
 	 if (( ${ld} > 0 )); then
 		 if [[ "$ld" == "" ]] || (( 1 != `check_numeric_parameter "$ld"` )) || [[ "$ld" == "0" ]]; then
 			 printStartStop "$STARTMSG"
 			 rc=12;
 			 echo "$PROCNAME: Parameter value missed or invalid value \$ld: >$ld<!" | tee -a $LOG_RESOURCE
 			 echo ${USAGE} | tee -a $LOG_RESOURCE
 			 printStartStop "$STOPMSG" "with Exit-Code: <${rc}>"
 			 exit ${rc};
 		 fi
 ...
 function assembling_process
 {
	 local _parameters="";
 	 if (( $ld > 0 )); then
 		 _parameters="${_parameters} $LD_OPTION ";
 		 _parameters="${_parameters} $ld ";
 	 fi
 	 ...
 	 if (( ${#_parameters} > 0 )); then
 		 _parameters="${_parameters} $VZ_OPTION $vz $DEST_OPTION $dest $VON_OPTION $von $BIS_OPTION $bis"
 	 else
 		 _parameters="$VZ_OPTION $vz $DEST_OPTION $dest $VON_OPTION $von $BIS_OPTION $bis";
 	 fi
 ...
 ###############################################################################################
 . $(dirname $0)/loadroutingms.option
 ...
 . $(dirname $0)/../vars/loadroutingms.vars;
