#! /usr/bin/ksh
 ...
 #Creation-Time:	14:59
 #Purpose:			starts, stops and status the exemplar binary batchcontrol for presentation only
 ###############################################################################################
 ...
 #LIBRARY PATH 
 #readonly LIBRARY_PATH=${LIB_PATH}/marathon/ssuk-01.08.01.00/sharedlib/release:${LIB_PATH}/tcaccess/8.0.6.2/sharedlib/release
 #export LD_LIBRARY_PATH=${LIBRARY_PATH}:$LD_LIBRARY_PATH
 readonly BINARY=batchcontrol.bin
 ...
 {
 	 if [[ "$2" == "" ]]; then set $1 "06.08.08 14:59:15"; fi
 ...
 assembling_process;
 print_msg "$STARTMSG" "$*"
# export SUPPLYGIDATAFROMHOST_BRANCHNO=$branchno
# export SUPPLYGIDATAFROMHOST_DATE=`date +%Y-%m-%d`
