#! /usr/bin/ksh
 ...
 {
 	 if [[ "$2" == "" ]]; then set $1 "17.09.08 17:20:45"; fi
 ...
 function assembling_process
 {
	 local _parameters="";
 	 ...
 	 if (( ${#_parameters} > 0 )); then
 		 _parameters="${_parameters} $VZ_OPTION $vz $DEST_OPTION $dest $VON_OPTION $von"
 	 else
 		 _parameters="$VZ_OPTION $vz $DEST_OPTION $dest $VON_OPTION $von";
 	 fi
