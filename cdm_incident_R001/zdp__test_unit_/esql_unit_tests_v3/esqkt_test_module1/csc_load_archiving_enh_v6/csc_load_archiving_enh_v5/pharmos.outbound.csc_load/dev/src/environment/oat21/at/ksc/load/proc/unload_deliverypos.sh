#! /usr/bin/ksh
 ...
 #Purpose:           Starts, stops and status the exemplar binary unload_deliverypos for presentation only.    
 ###############################################################################################
 ...
 . $(dirname $0)/../vars/unload_deliverypos.vars

 #--------------------------------------------------------------------#
 # LIBRARY / BINARY VARIABLES
 #--------------------------------------------------------------------#
 readonly BINARY=unload_deliverypos.bin
 ...
 function assembling_process
 {
	 local _parameters="";
 	 ...
 	 if (( ${#_parameters} > 0 )); then
 		 _parameters="${_parameters} $STARTDATE_OPTION $startdate $ENDDATE_OPTION $enddate"
 	 else
 		 _parameters="$STARTDATE_OPTION $startdate $ENDDATE_OPTION $enddate";
 	 fi
 ...
 ###############################################################################################
 . $(dirname $0)/unload_deliverypos.option
