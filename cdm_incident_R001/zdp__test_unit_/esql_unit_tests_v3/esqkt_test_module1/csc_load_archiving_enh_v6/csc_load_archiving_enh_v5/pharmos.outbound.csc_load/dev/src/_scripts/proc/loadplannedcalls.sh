#! /usr/bin/ksh
 ...
 #Purpose:           Starts, stops and status the exemplar binary loadplannedcalls for presentation only.      
 ###############################################################################################
 ...
 #--------------------------------------------------------------------#
 readonly BINARY=loadplannedcalls.bin
 ...
 function eval_parameters
 {
	 ...
	 #--------------------------#
 	 ...
 	 if [[ "${ld}" == "" ]] || (( 1 != `check_string_parameter "${LD_OPTION}" "${ld}"` )); then
 		 printStartStop "${STARTMSG}"
 		 rc=12;
 	 ...
 	 if [[ "${db}" != "" ]]; then
 		 if [[ "${db}" == "" ]] || (( 1 != `check_string_parameter "${DB_OPTION}" "${db}"` )) || [[ "${db}" == "0" ]]; then
 			 printStartStop "${STARTMSG}"
 			 rc=12;
 ...
 function assembling_process
 {
	 local _parameters="";
 	 if [[ "$db" != "" ]]; then
 		 _parameters="${_parameters} $DB_OPTION ";
 		 _parameters="${_parameters} $db ";
 	 fi
 	 ...
 	 if (( ${#_parameters} > 0 )); then
 		 _parameters="${_parameters} $BRANCHNO_OPTION $branchno $LD_OPTION $ld"
 	 else
 		 _parameters="$BRANCHNO_OPTION $branchno $LD_OPTION $ld";
 	 fi
 ...
 ###############################################################################################
 . $(dirname $0)/loadplannedcalls.option
 ...
 . $(dirname $0)/../vars/loadplannedcalls.vars;
