...
#define PROJECTNAME	 "loadpromotion"
...
#define APPLICATION		   "loadpromotion"
...
int Start()
{
	...
	for(count=0;;count++)
	{
		EXEC SQL fetch Cselcpromo into :promo;
		SQL_ERRMSG("fetch Cselcpromo")
		if ( SQLCODE == 100 ) break;

		EXEC SQL open Cselupdate using :promo.PROMOTION_NO;
		SQL_ERRMSG("open Cselupdate")

		EXEC SQL fetch Cselupdate into :sPromono;
		SQL_ERRMSG("fetch Cselupdate")
		if ( SQLCODE == 100 )
		{
			EXEC SQL EXECUTE ins_promo using :promo.PROMOTION_NO, 
										:promo.NAME, 
										:promo.DESCRIPTION,
										:promo.PROMOTION_TYPE,
										:promo.ADDONPROMODISCOK,
										:promo.ADDONSTDDISCOK,
										:promo.PRIORITY,
										:promo.SNOTBYSALESEWEB;
			SQL_ERRMSG("ins_promo")
			ins++;
		}
		else
		{
			EXEC SQL EXECUTE upd_promo using :promo.PROMOTION_NO, 
										:promo.NAME, 
										:promo.DESCRIPTION,
										:promo.PROMOTION_TYPE,
										:promo.ADDONPROMODISCOK,
...
int Prepare()
{
	...
	/* cursor for setnotupdated */
	sprintf(Stmt,"select PROMOTION_NO from promotions "
			"for update;");

	EXEC SQL PREPARE sel_notupd FROM :Stmt;
	SQL_ERRMSG( "prepare sel_notupd" );
	exec sql declare Cselnotupd cursor with hold for sel_notupd;

	sprintf(Stmt,"update promotions "
			"set updatesign = '0' "
			"where current of Cselnotupd;");
	EXEC SQL PREPARE set_notupd FROM :Stmt;
	SQL_ERRMSG( "prepare set_notupd" );
	...
	sprintf(Stmt,"delete from promotions "
	...
	EXEC SQL PREPARE sel_update FROM :Stmt;
	SQL_ERRMSG( "prepare sel_update" );
	exec sql declare Cselupdate cursor with hold for sel_update;

	sprintf(Stmt,"update promotions set "
			PROMOTIONS_UPDLISTE
			",updatesign=? "
			"where current of Cselupdate;");
	EXEC SQL PREPARE upd_promo FROM :Stmt;
	SQL_ERRMSG( "prepare upd_promo" );
	
	/* insert */
	sprintf(Stmt,"insert into promotions ("
			PROMOTIONS_PELISTE
			",updatesign "
			") values "
			"(?, ?, ?, ?, ?, ?, ?, ?, '1');");
...
int Setnotupd()
{
	...
	EXEC SQL open Cselnotupd;
	SQL_ERRMSG("open Cselnotupd")

	// for each line
	for(count=0;;count++)
	{
		EXEC SQL fetch Cselnotupd into :sPromono;
		SQL_ERRMSG("fetch Cselnotupd")
		if ( SQLCODE == 100 ) break;

		EXEC SQL EXECUTE set_notupd;
		SQL_ERRMSG("set_notupd")
