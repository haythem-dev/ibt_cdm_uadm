#! /usr/bin/ksh
 ...
 #Purpose:           Starts, stops and status the exemplar binary stocklevel_fr for presentation only.         
 ###############################################################################################
 ...
 #LIBRARY PATH 
 readonly LIBRARY_PATH=${LIB_PATH}/marathon/ssuk-01.04.00.00/sharedlib/release:${LIB_PATH}/tcaccess/8.0.0.7/sharedlib/release
 export LD_LIBRARY_PATH=${LIBRARY_PATH}:${LD_LIBRARY_PATH}
 readonly BINARY=stocklevel_fr._sh
 ...
 function assembling_process
 {
	 local _parameters="";
 	 ...
 	 if (( ${#_parameters} > 0 )); then
 		 _parameters="${_parameters} $KSCDB_OPTION $kscdb $BRANCHNO_OPTION $branchno"
 	 else
 		 _parameters="$KSCDB_OPTION $kscdb $BRANCHNO_OPTION $branchno";
 	 fi
 ...
 ###############################################################################################
 . $(dirname $0)/stocklevel_fr.option
